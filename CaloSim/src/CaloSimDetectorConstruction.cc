...
G4VPhysicalVolume* CaloSimDetectorConstruction::Construct()
{
  ...
  G4double blockWidth=4.0*cm;
  //  G4double blockDepth=11.0*cm;
  G4double blockDepth=40.0*cm;
  ...
  const G4int nsV= caloWidth/SVertWidth; //20
  const G4int nsH= caloHeight/SHorizHeight; //16
...
//   G4LogicalVolume* logicSHoriz = new G4LogicalVolume(solidSHoriz,BC404,"SHorizLV");
  G4LogicalVolume* logicSVert = new G4LogicalVolume(solidSVert,Vacuum,"SVertLV");
  G4LogicalVolume* logicSHoriz = new G4LogicalVolume(solidSHoriz,Vacuum,"SHorizLV");  
  logicSVert->SetSensitiveDetector(aSD);
  logicSHoriz->SetSensitiveDetector(aSD);
  logicSVert->SetVisAttributes(whiteFrame);  logicSHoriz->SetVisAttributes(whiteFrame);
  logicSVert->SetVisAttributes(whiteFrame);  logicSHoriz->SetVisAttributes(whiteFrame);
  ...
  for(G4int xi=0;xi<nsV; xi++){
    ...
    physiSVert[xi] = new G4PVPlacement(0,positionSVert,"SVert",logicSVert,experimentalHall_phys,false,xi+1);
  }

  G4VPhysicalVolume* physiSHoriz[nsH];
  for(G4int xi=0;xi<nsH; xi++){
    ...
    physiSVert[xi] = new G4PVPlacement(0,positionSHoriz,"SHoriz",logicSHoriz,experimentalHall_phys,false,xi+1);
  }

  //sheet to represent the casing of the calo
  G4Box* solidSheet = new G4Box("Sheet",caloWidth/2,caloHeight/2,SVertDepth/2);
  G4LogicalVolume* logicSheet = new G4LogicalVolume(solidSheet,BC404,"SheetLV");
  G4ThreeVector positionSheet = G4ThreeVector(0,0,-caloDepth/2-SVertDepth/2);
  G4VPhysicalVolume* physiSheet = new G4PVPlacement(0,positionSheet,"Sheet",logicSheet, experimentalHall_phys,false,0);
