...
#include "SIDIS_He3_FAEC_electron_trigger_HallDYe.C"
#include "SIDIS_He3_FAEC_hadron_trigger_HallDYe.C"
#include "SIDIS_He3_LAEC_electron_trigger_HallDYe.C"
//#include "SIDIS_He3_FAEC_electron_trigger_HallDYe_3to1.C"
//#include "SIDIS_He3_LAEC_electron_trigger_HallDYe_3to1.C"
//#include "SIDIS_He3_FAEC_hadron_trigger_WiserJinHuang.C"
//#include "SIDIS_He3_FAEC_hadron_trigger_HallDYe_3to1.C"
...
#include "analysis_tree_solid_ec_all.C"
...
//spd 
const double trigger_threshold_spd_FA=0.425;         //in MeV
//const double trigger_threshold_spd_FA=0.5;         //zhiwen's orginal value in MeV
...
TFile *outputfile=new TFile("HGCoriginal_eff2_event_radius_pi0_tid.root", "recreate");

// prepare for outputs
// define histograms, output txt files etc...
        TH1F *Momentum_hist =new TH1F("Momentum_hist","Momentum;momentum(GeV);",110,0,11);
	...
	TH2F *hocc_lgc_2D=new TH2F("occ_lgc_2D","occupancy,LGC (rear view),event rate(kHz);PMT_{#phi};PMT_{#theta}",3,0,3,3,0,3);
        TH1F *x_hist=new TH1F("x_hist","rate;x",100,0,1);
        TH1F *Q2_hist=new TH1F("Q2_hist","rate;Q^{2}[GeV^{2}]",100,0,13);
        TH1F *z_hist=new TH1F("z_hist","rate;z",100,0,1);
        TH1F *Pt_hist=new TH1F("Pt_hist","rate;Pt",300,0,3);
        TH2F *Q2_vs_x_hist =new TH2F("Q2_vs_x_hist","Q2_vs_x_hist;x;Q^{2} [GeV^{2}]",100,0,1,220,0,13);

        TH1F *x_cut_hist=new TH1F("x_cut_hist","rate;x",100,0,1);
        TH1F *Q2_cut_hist=new TH1F("Q2_cut_hist","rate;Q^{2}[GeV^{2}]",100,0,13);
        TH1F *z_cut_hist=new TH1F("z_cut_hist","rate;z",100,0,1);
        TH1F *Pt_cut_hist=new TH1F("Pt_cut_hist","rate;Pt",300,0,3);
        TH2F *Q2_vs_x_cut_hist = new TH2F("Q2_vs_x_cut_hist","Q2_vs_x_cut_hist;x;Q^{2} [GeV^{2}]",100,0,1,220,0,13);
        TH2F *Q2_vs_x_pfcut_hist = new TH2F("Q2_vs_x_pfcut_hist","Q2_vs_x_pfcut_hist;x;Q^{2} [GeV^{2}]",100,0,1,220,0,13);

        TH1F *x_pcut_hist=new TH1F("x_pcut_hist","rate;x",100,0,1);
        TH1F *Q2_pcut_hist=new TH1F("Q2_pcut_hist","rate;Q^{2}[GeV^{2}]",100,0,13);
        TH1F *z_pcut_hist=new TH1F("z_pcut_hist","rate;z",100,0,1);
        TH1F *Pt_pcut_hist=new TH1F("Pt_pcut_hist","rate;Pt",300,0,3);
        TH2F *Q2_vs_x_pcut_hist = new TH2F("Q2_vs_x_pcut_hist","Q2_vs_x_pcut_hist;x;Q^{2} [GeV^{2}]",100,0,1,220,0,13);

        TH1F *x_pcut_LAEC_hist=new TH1F("x_pcut_LAEC_hist","rate;x",100,0,1);
        TH1F *Q2_pcut_LAEC_hist=new TH1F("Q2_pcut_LAEC_hist","rate;Q^{2}[GeV^{2}]",100,0,13);
        TH1F *z_pcut_LAEC_hist=new TH1F("z_pcut_LAEC_hist","rate;z",100,0,1);
        TH1F *Pt_pcut_LAEC_hist=new TH1F("Pt_pcut_LAEC_hist","rate;Pt",300,0,3);
        TH2F *Q2_vs_x_pcut_LAEC_hist = new TH2F("Q2_vs_x_pcut_LAEC_hist","Q2_vs_x_pcut_hist;x;Q^{2} [GeV^{2}]",100,0,1,220,0,13);


        TH2F *hhit_r_vs_p_2D=new TH2F("hhit_r_vs_p_2D","hit,Q2>1 rate(kHz);radius [cm];P [GeV]",340,80,250,110,0,11);
        TH2F *hhit_r_vs_fluxp_2D=new TH2F("hhit_r_vs_fluxp_2D","hit,Q2>1 rate(kHz);radius [cm];P [GeV]",340,80,250,110,0,11);
	...
	TH1F *hangle_FAEC_LASPD=new TH1F("hangle_FAEC_LASPD","hangle_FAEC_LASPD",720,-360,360);	
        TH1F *h_ECp_hist[6];
        TH1F *h_ECLGCp_cut_hist[6];
        TH1F *h_ECLGCSPDp_cut_hist[6];
        TH1F *h_ECp_cut_hist[6];
        for(int i=0;i<6;i++){
         h_ECp_hist[i]=new TH1F(Form("h_ECp_Radius_index%d_hist",i),Form("h_ECp_Radius_index%d_hist",i),20,1,9);
         h_ECp_cut_hist[i]=new TH1F(Form("h_ECp_Radius_index%d_cut_hist",i),Form("h_ECp_Radius_index%d_cut_hist",i),20,1,9);
         h_ECLGCp_cut_hist[i]=new TH1F(Form("h_ECLGCp_Radius_index%d_cut_hist",i),Form("h_ECLGCp_Radius_index%d_cut_hist",i),20,1,9);
         h_ECLGCSPDp_cut_hist[i]=new TH1F(Form("h_ECLGCSPDp_Radius_index%d_cut_hist",i),Form("h_ECLGCSPDp_Radius_index%d_cut_hist",i),20,1,9);
 
         h_ECp_hist[i]->Sumw2();
         h_ECp_cut_hist[i]->Sumw2();
         h_ECLGCp_cut_hist[i]->Sumw2();
         h_ECLGCSPDp_cut_hist[i]->Sumw2();
         }
	//LGC
	...
	TH1F *h_counter_h_FA_EC_spd_mrpc=new TH1F("h_counter_h_FA_EC_spd_mrpc","h counter rate on at forward angle", 10,0,10);
        TH2F *FAEC_X_vs_Y =new TH2F("FAEC_X_vs_Y","X vs Y on FAEC; X [cm];Y[cm]", 600,-300,300,600,-300,300);	
        TH2F *LAEC_X_vs_Y =new TH2F("LAEC_X_vs_Y","X vs Y on LAEC;X [cm];Y[cm]", 600,-300,300,600,-300,300);
        TH2F *FAEC_X_vs_Y_h =new TH2F("FAEC_X_vs_Y_h","X vs Y on FAEC; X [cm];Y[cm]", 600,-300,300,600,-300,300);
        TH2F *LAEC_X_vs_Y_h =new TH2F("LAEC_X_vs_Y_h","X vs Y on LAEC;X [cm];Y[cm]", 600,-300,300,600,-300,300);
	...
	TH2F *hgen_ThetaP=new TH2F("gen_ThetaP","generated events;vertex #theta (deg);vertex P (GeV)",100,0,50,110,0,11);  
        TH2F *hgen_ThetaP_trigger=new TH2F("gen_ThetaP_trigger","generated events;vertex #theta (deg);vertex P (GeV)",100,0,50,110,0,11);
	...
	TH1F * Diff_EC_R = new TH1F("Diff_EC_R","Diff_EC_R",60,0,300);
        TH1F * Diff_EC_R_LAL = new TH1F("Diff_EC_R_LAL","Diff_EC_R_LAL",60,0,300);
// 	const int n=15;
// 	char *detname[n]={"GEM 1","GEM 2","GEM 3","GEM 4","GEM 5","GEM 6","LGC","HGC","FASPD","LASPD","FAEC","LAEC","FAMRPC","FAMUON","LAMUON"};	
	...
	TRandom3 rand;
	rand.SetSeed(0);
        LoadEC_map("/work/halla/solid/tianye/solid_simulation/solid/subsystem/ec/layout/map_FAEC_ANL_20130628.txt");	
  ...
  TVector3 ei_3vec(0,0,0);
  TLorentzVector pion_4vec(0,0,0,0);
  TVector3 pion_3vec(0,0,0);
  TLorentzVector He_4vec(0,0,0,0);
  TVector3 He_3vec(0,0,0);
  TLorentzVector PX(0,0,0,0);
  TLorentzVector q2_4vec(0,0,0,0);
...
double y=0.0;
double epsilon=0.0;
double pif_theta=0.0;
He_4vec.SetXYZM(0,0,0,Mp);
ei_4vec.SetXYZM(0,0,11000,0.511);
Int_t Ri;
  /*Lsidis mysidis; 
  mysidis.SetNucleus(1, 0);
...
// 	            cout << gen_pid->at(j) << " " << gen_px->at(j) << " " << gen_py->at(j) << " " << gen_pz->at(j) << " " << gen_vx->at(j) << " " << gen_vy->at(j) << " " << gen_vz->at(j) << endl; 
      ...
      if(pid_gen==11){
       ef_3vec.SetXYZ(px_gen,py_gen,pz_gen);
       ef_4vec.SetXYZM(px_gen,py_gen,pz_gen,0.511);
       ef_theta=ef_4vec.Theta()*DEG;
       q2_4vec = ei_4vec-ef_4vec;
      // Q2= -q2_4vec.Mag2();
        W_4vec = q2_4vec+He_4vec;
        Xbj=-q2_4vec.Mag2()/(2*He_4vec*q2_4vec);
	 }
    if(abs(pid_gen)==211){
       //cout<<"proton="<<pid_gen<<endl;
       pion_3vec.SetXYZ(px_gen,py_gen,pz_gen);
       pion_4vec.SetXYZM(px_gen,py_gen,pz_gen,139.57);
       pif_theta=pion_4vec.Theta()*DEG;
      }
   }		
 // mysidis.SetInitialState(ei_4vec, He_4vec);


//  mysidis.SetFinalState(ef_4vec, pion_4vec);

  //mysidis.CalculateVariables();
     PX = q2_4vec+ He_4vec - pion_4vec;//calculate final X 4-momentum
     z = (He_4vec*pion_4vec) / (He_4vec*q2_4vec);
     gamma = 2.0 * Xbj * Mp / sqrt(Q2);
     y = (He_4vec*q2_4vec) / (He_4vec*ei_4vec);
     epsilon = (1.0 - y - 0.25 * gamma * gamma * y * y) / (1.0 - y + 0.5 * y * y + 0.25 * gamma * gamma * y * y);
     double Pt2 = - (pion_4vec * pion_4vec) + 2.0 * (He_4vec * pion_4vec) * (q2_4vec * pion_4vec) / (He_4vec * q2_4vec) / (1.0 + gamma * gamma) - gamma * gamma / (1.0 + gamma * gamma) * (pow(q2_4vec * pion_4vec, 2) / Q2 - pow(He_4vec * pion_4vec, 2) / (Mp * Mp));
     Pt = sqrt(Pt2);
     kinematiccut = false; 
      x_hist->Fill(Xbj,rate);
     Q2_hist->Fill(Q2/1000000,rate);
      z_hist->Fill(z,rate);
     Pt_hist->Fill(Pt/1000,rate);
     Q2_vs_x_hist->Fill(Xbj,Q2/1000000,rate);
    // cout<<"Q2="<<Q2<<"   "<<"Pt="<<Pt/1000<<endl;
     if(Q2/1000000 > 1 && 0.3<z && z<0.7 && W_4vec.Mag()/1000>2.3 && PX.Mag()/1000>1.6 ){
		kinematiccut = true; 
      x_cut_hist->Fill(Xbj,rate);
     Q2_cut_hist->Fill(Q2/1000000,rate);
      z_cut_hist->Fill(z,rate);
     Pt_cut_hist->Fill(Pt/1000,rate);
     Q2_vs_x_cut_hist->Fill(Xbj,Q2/1000000,rate);
     }	 
// 		if (vz_gen<-3550 || -3450<vz_gen) continue;
   ...
   //if(kinematiccut){	
		...
		hit_r=sqrt(pow(flux_avg_x->at(j),2)+pow(flux_avg_y->at(j),2))/1e1; //mm to cm
		...
		double hit_phi=atan2(hit_y,hit_x)*DEG;       //rad to  deg
		 hit_p=sqrt(flux_px->at(j)*flux_px->at(j)+flux_py->at(j)*flux_py->at(j)+flux_pz->at(j)*flux_pz->at(j))/1e3;  //MeV to GeV
...
// 		  if (0<=hit_id && hit_id<=11) hflux_hitxy[hit_id]->Fill(flux_avg_x->at(j)/10.,flux_avg_y->at(j)/10.);
	    //       else cout << "flux_id->at(j) " << flux_id->at(j) << endl;
                //double edep_6p1E=0;
                //int trigger_ec=0;
                //process_tree_solid_ec(tree_solid_ec,hit_ec_preshower_FA,hit_ec_preshower_LA,hit_ec_shower_FA,hit_ec_shower_LA,hit_r,edep_6p1E,trigger_ec);		  
		  //check hit on EC and find sec_ec
		  if(hit_id==10){   //FAEC 
		    if (hit_r<rin_cut_FA || hit_r>rout_cut_FA) continue; //trigger cut on R
                    //if(trigger_ec==1){
                   // cout<<"event="<<i<<"trigger_ec="<<trigger_ec<<endl;
                   // }
		    int sec_ec=0;
...
// 	    	cout << " hit_phi " << hit_phi << " sec_ec " << sec_ec << endl;	
		    ...
		    if(Is_SIDIS_He3){
                    if(flux_tid->at(j)==1){
                    for (Int_t i =0; i<5;i++){
                     if (hit_r>trig_low_R[i] && hit_r<=trig_high_R[i]){
                         h_ECp_hist[i]->Fill(hit_p);
                     }
                    }
                      h_ECp_hist[5]->Fill(hit_p); 
                    }   
// 		    double hit_r_tmp;
...
/*		    if(abs(int(flux_pid->at(j))) == 11)		      EC_efficiency=get_forward_electron_trigger_e_eff(hit_r_tmp, hit_p);		      
		   ...
		   //if(hit_r>270){
                    //     cout<<"hit_r="<<hit_r<<endl;
                   // }
               /* x_pcut_hist->Fill(Xbj,rate);
                Q2_pcut_hist->Fill(Q2/1000000,rate);
                z_pcut_hist->Fill(z,rate);
                Pt_pcut_hist->Fill(Pt/1000,rate);
                Q2_vs_x_pcut_hist->Fill(Xbj,Q2/1000000,rate);*/
		    ...
		    if(abs(int(flux_pid->at(j))) == 11)		      EC_efficiency=get_forward_electron_trigger_e_eff(hit_r, hit_p);		      
                    ...
                    else if(int(flux_pid->at(j)) == 2112)                     EC_efficiency=0.5*get_forward_electron_trigger_pi_eff(hit_r, hit_p); //neutron
		    else {EC_efficiency=0; 
		      if (Is_debug) cout << "unknown particle pid" << flux_pid->at(j) << endl;	      
		    }
		    }
...
// 		      if (Is_passGEM==false) EC_efficiency=0;
			...
			if (Is_singlefile){
			  if(Is_pi0){
			    if(abs(flux_pid->at(j))==11 /*&& flux_tid->at(j)<8 && flux_mpid->at(j)==22*/) Is_ok=true;
			  }
			  else {
		      ...
		      if(Is_ok){						
                      // if(Q2 > 1 && W>2.0){
                       hhit_r_vs_p_2D->Fill(hit_r, p_gen/1e3,rate);
                       hhit_r_vs_fluxp_2D->Fill(hit_r, hit_p,rate);
                       hgen_ThetaP_trigger->Fill(theta_gen,p_gen/1e3,rate); 
                   //  }
                     /* for (Int_t i =0; i<5;i++){
                       if (hit_r>trig_low_R[i] && hit_r<=trig_high_R[i]){
                         h_ECp_cut_hist[i]->Fill(hit_p);
                       }
                      }
                       h_ECp_cut_hist[5]->Fill(hit_p);*/
...
// 		      EC_efficiency=1;
// 		    }
		   //cout<<"trigger_h_FA_EC="<<EC_efficiency<<"pid="<<flux_pid->at(j) << endl; 
		    //check to make sure eff is ok
			...
			if (Is_singlefile){
		      ...
		      trigger_h_FA_EC_vz[counter_h_FA_EC-1]=hit_vz;		      
               ...
               // double edp_6p1E_LAL=0;
               // int trigger_ec_LAL=0;
               // process_tree_solid_ec_LAL(tree_solid_ec,hit_ec_preshower_FA,hit_ec_preshower_LA,hit_ec_shower_FA,hit_ec_shower_LA,hit_r,edp_6p1E_LAL,trigger_ec_LAL);		  
		  if(hit_id==11){   //LAEC 
	    ...
	    // 	cout << " hit_phi " << hit_phi << " sec_ec " << sec_ec << endl;	
		    ...
		    //if(hit_r>150){
                  ...
                  /*if(ef_3vec.Mag()/1000>3.5){
                   ...
                   if(ef_3vec.Mag()/1000<3.5 && ef_3vec.Mag()/1000>3.0){
                    Q2_vs_x_pfcut_hist->Fill(Xbj,Q2/1000000,rate);
                    }
...
// 		hangle_FAEC_FASPD->Fill(hit_phi_FAEC-hit_phi_FASPD);
		...
		double edep_6p1E=0;
                ...
                double EC_R=0;
                int ntrigsecs_FA=0;
                process_tree_solid_ec(tree_solid_ec,hit_ec_preshower_FA,hit_ec_preshower_LA,hit_ec_shower_FA,hit_ec_shower_LA,edep_6p1E,ntrigsecs_FA,trigger_ec,EC_R);
                double edp_6p1E_LAL=0;
                int trigger_ec_LAL=0;
                double EC_R_LAL=0;
                int ntrigsecs_LA=0;
                process_tree_solid_ec_LAL(tree_solid_ec,hit_ec_preshower_FA,hit_ec_preshower_LA,hit_ec_shower_FA,hit_ec_shower_LA,edp_6p1E_LAL,ntrigsecs_LA,trigger_ec_LAL,EC_R_LAL);
                int pass_FAEC=0;
                if(ntrigsecs_FA){
                        pass_FAEC=1;
                 }else{
                ...
                if(pass_FAEC){
                //cout<<"event="<<i<<"trigger_ec="<<trigger_ec<<"hit_p"<<p_gen<<"hit_r="<<hit_r<<"edep_6p1E="<<edep_6p1E<<"E_R="<<EC_R<<endl;
		Diff_EC_R->Fill(EC_R,rate/1e3);
                }
                if(pass_LAEC){
                //  cout<<"event="<<i<<"edp_6p1E_LAL="<<edp_6p1E_LAL<<"trigger_ec_LAL="<<trigger_ec_LAL<<endl;
                   Diff_EC_R_LAL->Fill(EC_R_LAL,rate/1e3);
                 }
               // if(trigger_ec_LAL==1){
               // cout<<"event="<<i<<"edp_6p1E_LAL="<<edp_6p1E_LAL<<"trigger_ec_LAL="<<trigger_ec_LAL<<endl;
                //}		
		for(int index=0;index<2000;index++){
		  ...
		  if(hit_ec_shower_FA[index]>0){
                     hhit_ec_shower_FA->Fill(index,hit_ec_shower_FA[index]*rate/1e3);
                     //Diff_EC_R->Fill(EC_R,rate/1e3);
                  }
		...
		for(int index=0;index<2000;index++){
		  ...
		  if(hit_ec_shower_LA[index]>0){
                    hhit_ec_shower_LA->Fill(index,hit_ec_shower_LA[index]*rate/1e3);
                   //  Diff_EC_R_LAL->Fill(EC_R_LAL,rate/1e3);
                  //if(trigger_ec_LAL==1){
                  //cout<<"event="<<i<<"edp_6p1E_LAL="<<edp_6p1E_LAL<<"trigger_ec_LAL="<<trigger_ec_LAL<<endl;
               // }
...
// 		    h_trigger_e_FA_EC->Fill(trigger_e_FA_EC_r[i_e_FA_EC], rate); 
                     for (Int_t i =0; i<5;i++){
                       if (trigger_e_FA_EC_r[i_e_FA_EC]>trig_low_R[i] && trigger_e_FA_EC_r[i_e_FA_EC]<=trig_high_R[i]){
                         h_ECp_cut_hist[i]->Fill(trigger_e_FA_EC_p[i_e_FA_EC]);
                       }
                      }
                       h_ECp_cut_hist[5]->Fill(trigger_e_FA_EC_p[i_e_FA_EC]);

		    /*for (Int_t i =0; i<5;i++){
                       if (trigger_e_FA_EC_r[i_e_FA_EC]>trig_low_R[i] && trigger_e_FA_EC_r[i_e_FA_EC]<=trig_high_R[i]){
                         h_ECp_cut_hist[i]->Fill(trigger_e_FA_EC_p[counter_e_FA_EC-1],rate/counter_e_FA_EC);
                       }
                      }
                       h_ECp_cut_hist[5]->Fill(trigger_e_FA_EC_p[counter_e_FA_EC-1],rate/counter_e_FA_EC);*/
		  ...
		  if (trigger_lgc[last_sec-1]==1 || trigger_lgc[this_sec-1]==1 || trigger_lgc[next_sec-1]==1){
			...
			h_trigger_e_FA_EC_lgc->Fill(trigger_e_FA_EC_r[i_e_FA_EC], rate/counter_e_FA_EC_lgc);		      
                    for (Int_t i =0; i<5;i++){
                       if (trigger_e_FA_EC_r[i_e_FA_EC]>trig_low_R[i] && trigger_e_FA_EC_r[i_e_FA_EC]<=trig_high_R[i]){
                         h_ECLGCp_cut_hist[i]->Fill(trigger_e_FA_EC_p[i_e_FA_EC]);
                       }
                      }
                       h_ECLGCp_cut_hist[5]->Fill(trigger_e_FA_EC_p[i_e_FA_EC]);
...
// 			    if (hit_vz<95) h_trigger_e_FA_EC_lgc_spd->Fill(trigger_e_FA_EC_r[i_e_FA_EC], rate/counter_e_FA_EC_lgc_spd);	
			    h_trigger_e_FA_EC_lgc_spd->Fill(trigger_e_FA_EC_r[i_e_FA_EC], rate/counter_e_FA_EC_lgc_spd);
                    for (Int_t i =0; i<5;i++){
                       if (trigger_e_FA_EC_r[i_e_FA_EC]>trig_low_R[i] && trigger_e_FA_EC_r[i_e_FA_EC]<=trig_high_R[i]){
                         h_ECLGCSPDp_cut_hist[i]->Fill(trigger_e_FA_EC_p[i_e_FA_EC]);
                       }
                      }
                       h_ECLGCSPDp_cut_hist[5]->Fill(trigger_e_FA_EC_p[i_e_FA_EC]);
	
// 			    h_trigger_e_FA_EC_lgc_spd->Fill(trigger_e_FA_EC_r[i_e_FA_EC], rate);	
			    //h_ECp_cut_hist->Fill(hit_p);  
			    int sector_mrpc=0,block_mrpc=0;
// 			    if(find_id_mrpc_FA(hit_phi,hit_r,sector_mrpc,block_mrpc)){
// 			      if(trigger_mrpc_FA[sector_mrpc-1][block_mrpc-1]==1){			   							
				h_trigger_e_FA_EC_lgc_spd_mrpc->Fill(trigger_e_FA_EC_r[i_e_FA_EC], rate/counter_e_FA_EC_lgc_spd_mrpc);	
                             //if(Q2/1000000 > 1 && 0.3<z && z<0.7 && W_4vec.Mag()/1000>2.3 && PX.Mag()/1000>1.6){
                               ...
                               Pt_pcut_hist->Fill(Pt/1000,rate);
                               Q2_vs_x_pcut_hist->Fill(Xbj,Q2/1000000,rate);
                               FAEC_X_vs_Y->Fill(trigger_e_FA_EC_x[i_e_FA_EC],trigger_e_FA_EC_y[i_e_FA_EC],rate);			
                            // }
...
// 			    cout << "event " << i<< " counter_e_LA " <<  counter_e_LA_EC_spd << "\t" << trigger_e_LA_EC_pid[i_e_LA_EC] << " " << trigger_e_LA_EC_p[i_e_LA_EC] << " " << trigger_e_LA_EC_sec[i_e_LA_EC] << " " << 	trigger_e_LA_EC_r[i_e_LA_EC] << endl;			   
		...
		for(int i_e_LA_EC=0; i_e_LA_EC<counter_e_LA_EC; i_e_LA_EC++){
		    ...
		    if(find_id_spd_LA(hit_phi,hit_r,sector_spd)){
		      if(trigger_spd_LA[sector_spd-1]==1){			   
                      // if(/*ef_3vec.Mag()/1000>3.5 &&*/ Q2/1000000 > 1 && 0.3<z && z<0.7 && W_4vec.Mag()/1000>2.3 && PX.Mag()/1000>1.6){
                        x_pcut_hist->Fill(Xbj,rate);
                        Q2_pcut_hist->Fill(Q2/1000000,rate);
                        z_pcut_hist->Fill(z,rate);
                        Pt_pcut_hist->Fill(Pt/1000,rate);
                        Q2_vs_x_pcut_hist->Fill(Xbj,Q2/1000000,rate);
                        x_pcut_LAEC_hist->Fill(Xbj,rate);
                        Q2_pcut_LAEC_hist->Fill(Q2/1000000,rate);
                        z_pcut_LAEC_hist->Fill(z,rate);
                        Pt_pcut_LAEC_hist->Fill(Pt/1000,rate);
                        Q2_vs_x_pcut_LAEC_hist->Fill(Xbj,Q2/1000000,rate);
                        LAEC_X_vs_Y->Fill(trigger_e_LA_EC_x[i_e_LA_EC],trigger_e_LA_EC_y[i_e_LA_EC],rate);
                      //  }										    
...
// 		      cout << "event " << i<< " counter_e_FA_h_FA " << counter_e_FA_h_FA << "\t" << trigger_e_FA_EC_pid[i_e_FA_EC_good[k]] << " " << trigger_h_FA_EC_pid[i_h_FA_EC_good[j]] << " " <<  trigger_e_FA_EC_sec[i_e_FA_EC_good[k]] << " " << trigger_h_FA_EC_sec[i_h_FA_EC_good[j]] << " " <<		      trigger_e_FA_EC_p[i_e_FA_EC_good[k]] << " " << trigger_h_FA_EC_p[i_h_FA_EC_good[j]] << endl;			      		      
		...
		for(int k=0; k<counter_e_FA_EC_lgc_spd_mrpc; k++){		
		  for(int j=0; j<counter_h_FA_EC_spd_mrpc; j++){
		    double dist=sqrt(pow(trigger_e_FA_EC_y[i_e_FA_EC_good[k]]-trigger_h_FA_EC_y[i_h_FA_EC_good[j]],2)+pow(trigger_e_FA_EC_x[i_e_FA_EC_good[k]]-trigger_h_FA_EC_x[i_h_FA_EC_good[j]],2));
		    if (dist>=threshold_distance){
                       FAEC_X_vs_Y_h->Fill(trigger_e_FA_EC_x[i_e_FA_EC_good[k]],trigger_e_FA_EC_y[i_e_FA_EC_good[k]],rate/counter_e_FA_h_FA);
                       h_trigger_e_FA_h_FA->Fill(trigger_e_FA_EC_r[i_e_FA_EC_good[k]],rate/counter_e_FA_h_FA);
		     }
                    }
		}			
...
// 		      cout << "event " << i<< " counter_e_LA_h_FA " << counter_e_LA_h_FA << "\t" << trigger_e_LA_EC_pid[i_e_LA_EC_good[k]] << " " << trigger_h_FA_EC_pid[i_h_FA_EC_good[j]] << " " << trigger_e_LA_EC_sec[i_e_LA_EC_good[k]] << " " << trigger_h_FA_EC_sec[i_h_FA_EC_good[j]] << " " << trigger_e_LA_EC_sec[i_e_LA_EC_good[k]] << " " << trigger_h_FA_EC_sec[i_h_FA_EC_good[j]] << endl;	 		      
		...
		for(int k=0; k<counter_e_LA_EC_spd; k++){		
		  for(int j=0; j<counter_h_FA_EC_spd_mrpc; j++){
                    LAEC_X_vs_Y_h->Fill(trigger_e_LA_EC_x[i_e_LA_EC_good[k]],trigger_e_LA_EC_y[i_e_LA_EC_good[k]],rate/counter_e_LA_h_FA);
		    h_trigger_e_LA_h_FA->Fill(trigger_e_LA_EC_r[i_e_LA_EC_good[k]],rate/counter_e_LA_h_FA);
		  }
		}		
		
       //} //end kinematic cut loop
	} //end event loop
	
	} //end loop time
       TH1F *h_ECp_eff_hist[6];
       TH1F *h_ECLGCp_eff_hist[6];
      TH1F *h_ECLGCSPDp_eff_hist[6];
      for(int i=0;i<6;i++){
       h_ECp_eff_hist[i]= (TH1F*)h_ECp_cut_hist[i]->Clone("h_ECp_eff_hist[i]");
       h_ECp_eff_hist[i]->SetName(Form("h_ECp_eff_Radius_index%d_hist",i));
       h_ECp_eff_hist[i]->Sumw2();
       h_ECp_eff_hist[i]->Divide(h_ECp_hist[i]);
       h_ECLGCp_eff_hist[i]= (TH1F*)h_ECLGCp_cut_hist[i]->Clone("h_ECLGCp_eff_hist[i]");
       h_ECLGCp_eff_hist[i]->SetName(Form("h_ECLGCp_eff_Radius_index%d_hist",i));
       h_ECLGCp_eff_hist[i]->Sumw2();
       h_ECLGCp_eff_hist[i]->Divide(h_ECp_hist[i]);
       h_ECLGCSPDp_eff_hist[i]= (TH1F*)h_ECLGCSPDp_cut_hist[i]->Clone("h_ECLGCSPDp_eff_hist[i]");
       h_ECLGCSPDp_eff_hist[i]->SetName(Form("h_ECLGCSPDp_eff_Radius_index%d_hist",i));
       h_ECLGCSPDp_eff_hist[i]->Sumw2();
       h_ECLGCSPDp_eff_hist[i]->Divide(h_ECp_hist[i]);
      }
/*
...
TCanvas *c_Q2x = new  TCanvas("c_Q2x","c_Q2x",1000,1000);
...
cout<<"h_trigger_e_FA_EC in kHz response: "<<Diff_EC_R->Integral(1,60)<<endl;
cout<<"h_trigger_e_LA_EC in kHz response: "<<Diff_EC_R_LAL->Integral(1,60)<<endl;
cout<<"h_trigger_e_FA_EC in kHz: "<<h_trigger_e_FA_EC->Integral(1,300)/1e3<<endl;
...
cout<<"h_trigger_e_FA_h_FA in kHz: "<<h_trigger_e_FA_h_FA->Integral(1,60)/1e3<<"event number:"<<h_trigger_e_FA_h_FA->GetEntries()<< endl; 
cout<<"h_trigger_e_LA_h_FA in kHz: "<<h_trigger_e_LA_h_FA->Integral(1,60)/1e3<<"event number:"<<h_trigger_e_LA_h_FA->GetEntries()<< endl;
...
//outputfile->Flush();
        outputfile->cd();
        hhit_r_vs_p_2D->Write();
        for(int i=0;i<6;i++){
        h_ECp_eff_hist[i]->Write();
        h_ECLGCp_eff_hist[i]->Write();
        h_ECLGCSPDp_eff_hist[i]->Write();
        }
        hhit_r_vs_fluxp_2D->Write();
 	Momentum_hist->SetDirectory(outputfile);
 	outputfile->Write();
        outputfile->Close();
/*
