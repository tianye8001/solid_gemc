...
Bool_t process_tree_solid_hgc(TTree *tree_solid_hgc,double *hit_hgc,Int_t *trigger_hgc, Int_t &ntrigsecs_hgc, Int_t PMTthresh_hgc, Int_t PEthresh_hgc,Int_t ch_hgc,TH1F *htime_photon,Int_t &photon_mtid,TH1F *hwl_photon,TH1F *hr_photon)
{ 
  	TRandom3 rand;
	rand.SetSeed(0);
...
// 	  cout << "E_photon " << E_photon << endl;
	  hwl_photon->Fill(1.24/E_photon*1e3); // from ev to nm
	  hr_photon->Fill(sqrt(solid_hgc_avg_lx->at(j)*solid_hgc_avg_lx->at(j)+solid_hgc_avg_ly->at(j)*solid_hgc_avg_ly->at(j))/10.);
	  double weight=0;
	  bool Is_pass=false;
	  for (Int_t k=0;k<n;k++) {	      
