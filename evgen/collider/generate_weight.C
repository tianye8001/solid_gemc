...
void generate_weight(string file_name){
	TFile *file=new TFile(file_name.c_str(),"update"); 	
	
// 	TFile *file=new TFile("collider_3he_pim_11_0_1_0.root","update"); 	
// 	TFile *file=new TFile("collider_3he_pim_11_0_2_0.root","update"); 	
...
// 	double nsim=0;
	...
	double phase_hp=0,phase_hm=0;	
	T->SetBranchAddress("nsim",&nsim);
	T->SetBranchAddress("dxs_hp",&dxs_hp);
	T->SetBranchAddress("dxs_hm",&dxs_hm);
	T->SetBranchAddress("phase_hp",&phase_hp);
	T->SetBranchAddress("phase_hm",&phase_hm);	
...
// 		weight_hm=dxs_hm*Phase_space/N_simulate;
		weight_hp=dxs_hp*phase_hp/N_simulate;   		
		weight_hm=dxs_hm*phase_hm/N_simulate;		
