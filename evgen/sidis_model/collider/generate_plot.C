...
void generate_plot(TString particle,int hadron_flag,int DIRC_num){
//particle: pion    kaon
...
//DIRC_num 4: (2 sides)  6: (3 sides)   8: (4 sides)   no meaning for pion case 
	...
	if(particle=="pion"){
		T->Add("../collider_3he_pip_11_0_1_1.root");   // save Q2<10, pt<1
	...
	//electron acceptance	
	TFile *file_negative=new TFile("acceptance_solid_CLEO_SIDIS_3he_negative_output.root");
	TFile *file_hadron=0;
	if(particle=="kaon"){
		if(DIRC_num==4){
