...
int main(Int_t argc, char *argv[]){
  ...
  if (argc != 8) {
    cout << "usage: ./sidis_gen #ele_mom #ion_mom #target #particle #fileno #events config" << endl;
    ...
    cout << "config is 'MEIC' or 'SoLID' which needs ion_mom=0" << endl;
    cout << "note for MEIC: e- goes alone -z,ion goes 50mrad relative to +z with py=0, px>0, customized theta and angle range within code" << endl;
    cout << "note for SoLID: e- goes along +z, ion_mom=0, customized theta and angle range within code " << endl;
  }else{
    //initialize
    ...
    TLorentzVector *P4_ini_ele,*P4_ini_ion;
    if (config=="MEIC") {
      P4_ini_ele = new TLorentzVector(0.,0.,-momentum_ele,sqrt(momentum_ele*momentum_ele + mass_e*mass_e));
      P4_ini_ion = new TLorentzVector(momentum_ion*sin(50e-3),0.,momentum_ion*cos(50e-3),sqrt(momentum_ion*momentum_ion + mass_target*mass_target));
    }
    else if (config=="SoLID") {
      P4_ini_ele = new TLorentzVector(0.,0.,momentum_ele,sqrt(momentum_ele*momentum_ele + mass_e*mass_e));
      P4_ini_ion = new TLorentzVector(0.,0.,0,mass_target);
    }    
...
//     TVector3 vnboost(0.,0.,vn);
    TVector3 vnboost(vn*sin(50e-3),0.,vn*cos(50e-3));
...
//       cout << "nsim " << nsim << endl;      
      ...
      double ele_theta_max,ele_theta_min,ele_mom_max,ele_mom_min;
      double had_theta_max,had_theta_min,had_mom_max,had_mom_min;      
      if (config=="MEIC") {
	ele_theta_max=180.;
	ele_theta_min=0.;
	ele_mom_max=momentum_ele * 3.;
	ele_mom_min=0.7;
	had_theta_max=180.;
	had_theta_min=0.;
	had_mom_max=momentum_ele + momentum_ion;
	had_mom_min=0.7;
      }
      else if (config=="SoLID") {
	ele_theta_max=30.;
	ele_theta_min=7.;
	ele_mom_max=momentum_ele;
	ele_mom_min=0.5;
	had_theta_max=30.;
	had_theta_min=7.;
	had_mom_max=6.;
	...
	theta_gen = acos(gRandom->Uniform(cos(ele_theta_max/DEG),cos(ele_theta_min/DEG)));
	mom_gen = gRandom->Uniform(ele_mom_min,ele_mom_max);
	...
	theta_gen = acos(gRandom->Uniform(cos(had_theta_max/DEG),cos(had_theta_min/DEG)));
	mom_gen = gRandom->Uniform(had_mom_min,had_mom_max);
	phi_gen = gRandom->Uniform(0.,2.*PI);	
	// theta_gen = 1.8588846;
	//        phi_gen = 0.8115906;
	//        mom_gen = 2.3559179; 
	...
	double phasespace=(cos(had_theta_min/DEG)-cos(had_theta_max/DEG))*2*PI*(had_mom_max-had_mom_min)*(cos(ele_theta_min/DEG)-cos(ele_theta_max/DEG))*2*PI*(ele_mom_max-ele_mom_min);
...
// 	 cout << "here 1" << endl;  
	 if ( (config=="MEIC" && z>0.2&&z<0.8&&y>0.05&&y<0.8 && ((count[0]<number_of_events&&pt<=1.0&&Q2<=10.) 
...
// 	   cout << "nsim " << nsim << endl;
	   ...
	   cout << "count " << count[0] << "\t" << count[1] << "\t" << count[2] << "\t" << count[3] << "\t\t\t\t\t";   
	   cout << "nsim " << count_nsim[0] << "\t" << count_nsim[1] << "\t" << count_nsim[2] << "\t" << count_nsim[3] << "\r";
