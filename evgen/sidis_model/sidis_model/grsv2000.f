...
*                                                  GRID: 2.4395E-04 )
      ...
      IF (ISET.EQ.1) THEN
       IIREAD=11       
       OPEN(UNIT=11,FILE=
     $      PREFIX//'GRSV-GRID/std2000_nlo_g1.grid',STATUS='OLD')
      ELSE IF (ISET.EQ.2) THEN
       IIREAD=22
       OPEN(UNIT=22,FILE=
     $      PREFIX//'GRSV-GRID/val2000_nlo_g1.grid',STATUS='OLD')
      ELSE IF (ISET.EQ.3) THEN
       IIREAD=33       
       OPEN(UNIT=33,FILE=
     $      PREFIX//'GRSV-GRID/std2000_lo_g1.grid',STATUS='OLD')
      ELSE IF (ISET.EQ.4) THEN
       IIREAD=44
       OPEN(UNIT=44,FILE=
     $      PREFIX//'GRSV-GRID/val2000_lo_g1.grid',STATUS='OLD')
      ELSE
        WRITE(6,93)
...
*...INTERPOLATION :
      XT(1) = DLOG(X)
      XT(2) = DLOG(Q2)
      U = DFINTGRSV(NARG,XT,NA,ARRF,XUF) * (1.D0-X)**3 * X
      D = DFINTGRSV(NARG,XT,NA,ARRF,XDF) * (1.D0-X)**4 * X
      UB = DFINTGRSV(NARG,XT,NA,ARRF,XUBF) * (1.D0-X)**8 * X**0.5
      DB = DFINTGRSV(NARG,XT,NA,ARRF,XDBF) * (1.D0-X)**8 * X**0.5
      ST = DFINTGRSV(NARG,XT,NA,ARRF,XSF)  * (1.D0-X)**8 * X**0.5
      GL = DFINTGRSV(NARG,XT,NA,ARRF,XGF)  * (1.D0-X)**5 * X**0.5
      G1P = DFINTGRSV(NARG,XT,NA,ARRF,XG1P)  * (1.D0-X)**3
      G1N = DFINTGRSV(NARG,XT,NA,ARRF,XG1N)  * (1.D0-X)**3
 60   RETURN
      END
*
*...CERN LIBRARY ROUTINE E104 (INTERPOLATION) :
*
      FUNCTION DFINTGRSV(NARG,ARG,NENT,ENT,TABLE)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
   ...
   15 IFADR=IFADR*NENT(I)
      DFINTGRSV=DFINTGRSV+FAC*TABLE(IADR)
      IL=NARG
