...
const int Q2bin=10;
  ...
  for (int i=0;i<xbin;i++){
    for (int j=0;j<Q2bin;j++){
      char cmd[100];
      double x_min=x_low+0.05*i,x_max=x_low+0.05*(i+1);
      double Q2_min=Q2_low+1.*j,Q2_max=Q2_low+1.*(j+1);
      sprintf (cmd,"./main 2 1000. 11. %f %f %f %f",x_min,x_max,Q2_min,Q2_max);
      gSystem->Exec(cmd);
