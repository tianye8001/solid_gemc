...
my @Nsect = ($parameters{"Nsect1"},$parameters{"Nsect2"},$parameters{"Nsect3"},$parameters{"Nsect4"},$parameters{"Nsect5"},$parameters{"Nsect6"});

#in cm 
my @PlateZ = ($parameters{"PlateZ1"},$parameters{"PlateZ2"},$parameters{"PlateZ3"},$parameters{"PlateZ4"},$parameters{"PlateZ5"},$parameters{"PlateZ6"});
my @Rin    = ($parameters{"Rin1"},$parameters{"Rin2"},$parameters{"Rin3"},$parameters{"Rin4"},$parameters{"Rin5"},$parameters{"Rin6"});

# in deg
my @angle_width = (360./$parameters{"Nsect1"},360./$parameters{"Nsect2"},360./$parameters{"Nsect3"},360./$parameters{"Nsect4"},360./$parameters{"Nsect5"},360./$parameters{"Nsect6"});
my @offrot = ($parameters{"offrot1"},$parameters{"offrot2"},$parameters{"offrot3"},$parameters{"offrot4"},$parameters{"offrot5"},$parameters{"offrot6"});

#plane 1 and plane 2 using gem1, plane3 and plane4 use gem2, plane5 use gem1, plane6 use gem2
# in mm
my @gem_inner_half=($parameters{"gem1_inner"}/2,$parameters{"gem1_inner"}/2,$parameters{"gem2_inner"}/2,$parameters{"gem2_inner"}/2,$parameters{"gem1_inner"}/2,,$parameters{"gem2_inner"}/2);
my @gem_outer_half=($parameters{"gem1_outer"}/2,$parameters{"gem1_outer"}/2,$parameters{"gem2_outer"}/2,$parameters{"gem2_outer"}/2,$parameters{"gem1_outer"}/2,$parameters{"gem2_outer"}/2);
my @gem_height_half=($parameters{"gem1_height"}/2,$parameters{"gem1_height"}/2,$parameters{"gem2_height"}/2,$parameters{"gem2_height"}/2,$parameters{"gem1_height"}/2,$parameters{"gem2_height"}/2);

my @frame_inner_half=($parameters{"frame1_inner"}/2,$parameters{"frame1_inner"}/2,$parameters{"frame2_inner"}/2,$parameters{"frame2_inner"}/2,$parameters{"frame1_inner"}/2,$parameters{"frame2_inner"}/2);
my @frame_outer_half=($parameters{"frame1_outer"}/2,$parameters{"frame1_outer"}/2,$parameters{"frame2_outer"}/2,$parameters{"frame2_outer"}/2,$parameters{"frame1_outer"}/2,$parameters{"frame2_outer"}/2);
...
my @hittype = ("no","no","no","no","solid_gem","solid_gem","solid_gem","no","solid_gem","solid_gem","solid_gem","no","no","no","no","no","no","no","solid_gem","no","no","no","no","no");
 ...
 for(my $n=1; $n<=$Nplate; $n++)
 {
    ...
    #for overlaping sector
    my $PlateZ_tube=$PlateZ[$n-1]+$Dz/10+$PlateZ_dis/10/2;   
    my $Dz_tube=$Dz*2+$PlateZ_dis/2;
    
    my %detector=init_det();
    $detector{"name"}        = "$DetectorName\_$n";
    $detector{"mother"}      = "$DetectorMother" ;
    $detector{"description"} = $detector{"name"};
    $detector{"pos"}        = "0*cm 0*cm $PlateZ_tube*cm";
    $detector{"rotation"}   = "0*deg 0*deg 0*deg";
    $detector{"color"}      = "111111";
    $detector{"type"}       = "Tube";
    $detector{"dimensions"} = "$Rin_tube*cm $Rout_tube*cm $Dz_tube*mm 0*deg 360*deg";
    ...
    for( my $sec = 1; $sec <= $Nsect[$n-1]; $sec++ ){
      ...
      #for overlaping sector      
      my $sec_pos_z = 0;
      if ($sec % 2 == 0) {$sec_pos_z=$Dz+$PlateZ_dis/2;} # move even number sector in larger z
      else {$sec_pos_z=-($Dz+$PlateZ_dis/2);}  # move odd number sector in smaller z
      
	    my %detector=init_det();
	    $detector{"name"}        = "$DetectorName\_$n\_$sec";
	    $detector{"mother"}      = "$DetectorName\_$n";
	    $detector{"description"} = $detector{"name"};
	    $detector{"pos"}        = "$sec_pos_x*mm $sec_pos_y*mm $sec_pos_z*mm ";
...
# 	    $detector{"rotation"}   = "0*deg 0*deg 0*deg";
	    ...
	    $detector{"material"}   = "SL_gem_Kapton";
	    $detector{"mfield"}     = "no";
	    $detector{"ncopy"}      = 1;
	    $detector{"pMany"}       = 1;
	    $detector{"exist"}       = 1;
