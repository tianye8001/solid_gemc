# -*- coding: utf-8 -*-
"""
Created on Tue Aug  5 15:20:59 2025
...
# --- USER INPUT ---
local_pitch = -15  # What you want LOCALLY (e.g., -15 for "downward")
global_pitch_deg = -local_pitch  # Use this for Euler angle code! (i.e. positive)
...
# --- Position Calculation (geometry order) ---
positions = []
for i in range(num_wedges):
    bottom = bottom_points[i]
    theta = np.deg2rad(theta_zs_geom[i])
    axis = np.array([-np.sin(theta), np.cos(theta), 0])
    center = orig_center
    rot_center = rotate_about_axis(center, bottom, axis, local_pitch)
    positions.append(rot_center)

# --- Euler Angle Calculation (rotation order, to be shifted) ---
euler_angles = []
for i in range(num_wedges):
    # Rotation order: 12, 24, ..., 360
    theta_z_deg = (i+1) * z_increment_deg
    theta_z_rad = np.deg2rad(theta_z_deg)
    Ry_pitch = np.array([
        [np.cos(np.deg2rad(global_pitch_deg)), 0, np.sin(np.deg2rad(global_pitch_deg))],
        [0, 1, 0],
        [-np.sin(np.deg2rad(global_pitch_deg)), 0, np.cos(np.deg2rad(global_pitch_deg))]
    ])
    Rz = np.array([
...
# -- Flip X and Z for Perl convention --
euler_angles_perl = [(-x, y, -z) for (x, y, z) in euler_angles_shifted]

# --- Output Perl lines ---
print("\n# === Perl code for all 30 wedges (position and rotation matched to convention) ===\n")
for i in range(num_wedges):
    x, y, z = positions[i]
