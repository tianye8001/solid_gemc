...
# the original has to be made before copies, because GEMC make volume in alphabetic order of names, the origin name has to be before copies, one way to do it is to make it shorter
	  make_ec_module_shower(1);
          make_ec_module_supportsh(1);
	  make_ec_module_prescint(1);
	  make_ec_module_support(1);
...
{   
    ...
    if($id == 1){ $detector{"name"}        = "$DetectorName\_showe$id";}
    ...
    else {  $detector{"type"}       = "CopyOf $DetectorName\_showe1";}         
...
{   
    ...
    if($id == 1){ $detector{"name"}        = "$DetectorName\_prescin$id";}
    ...
    else {  $detector{"type"}       = "CopyOf $DetectorName\_prescin1";}      
    $detector{"material"}   = "$material_scint";
    $detector{"mfield"}     = "no";
    $detector{"ncopy"}      = 1;
    $detector{"pMany"}       = 1;
...
sub make_ec_module_support()
{
    ...
    if($id == 1){ $detector{"name"}        = "$DetectorName\_suppor$id";}
   ...
   if($id == 1){
    ...
    else {  $detector{"type"}       = "CopyOf $DetectorName\_suppor1";}
...
sub make_ec_module_supportsh()
{
    ...
    if($id == 1){ $detector{"name"}        = "$DetectorName\_supporsh$id";}
    else {$detector{"name"}        = "$DetectorName\_support$id";}
    $detector{"mother"}      = "$DetectorMother" ;
    $detector{"description"} = $detector{"name"};
   ...
   if($id == 1){
    ...
    else {  $detector{"type"}       = "CopyOf $DetectorName\_supporsh1";}
    $detector{"material"}   = "$material_support";
    $detector{"mfield"}     = "no";
    $detector{"ncopy"}      = 1;
    $detector{"pMany"}       = 1;
