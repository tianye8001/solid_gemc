...
my $material_gap="G4_TITANIUM_DIOXIDE";
my $material_support="G4_Al";
my $material_wrap="G4_MYLAR";
my $material_air = "G4_AIR";
...
my $Thickness_shield_scin 	= $parameters{"Thickness_shield_scin"};
my $Thickness_prescint 	= $parameters{"Thickness_prescint"};
my $Thickness_support 	= $parameters{"Thickness_support"};
my $Thickness_supportsh   = $parameters{"Thickness_supportsh"};
...
my $z_supportsh = $z_shower-$Thickness_shower/2-$Thickness_supportsh/2; #cm
my $z_support = $z_supportsh-$Thickness_supportsh/2-$Thickness_support/2;  #cm
...
my $z_shield_scin1 = $z_shield-$Thickness_shield/2-$Thickness_shield_scin/2-500;  #about 323.4cm
my $z_shield_scin2 = $z_shield_scin1-$Thickness_shield_scin/2-$Thickness_shield_scin/2-400;  #about 323.4cm
my $z_shield_scin3 = $z_shield_scin2-$Thickness_shield_scin/2-$Thickness_shield_scin/2-600;  #about 323.4cm


sub solid_PVDIS_ec_forwardangle
{
make_ec_shield();
make_ec_shield_scin1();
make_ec_shield_scin2();
make_ec_shield_scin3();
make_ec_module();
}
...
sub make_ec_shield_scin1
{
 my $id = $_[0];
 my $Dz    = $Thickness_shield_scin/2;

 my %detector=init_det();
 $detector{"name"}        = "$DetectorName\_shield_scin1";
 $detector{"mother"}      = "$DetectorMother";
 $detector{"description"} = $detector{"name"};
 $detector{"pos"}         = "0*cm 0*cm $z_shield_scin1*cm";
 $detector{"rotation"}    = "0*deg 0*deg 0*deg";
...
sub make_ec_shield_scin2
{
 my $id = $_[0];
 my $Dz    = $Thickness_shield_scin/2;

 my %detector=init_det();
 $detector{"name"}        = "$DetectorName\_shield_scin2";
 $detector{"mother"}      = "$DetectorMother";
 $detector{"description"} = $detector{"name"};
 $detector{"pos"}         = "0*cm 0*cm $z_shield_scin2*cm";
 $detector{"rotation"}    = "0*deg 0*deg 0*deg";
 $detector{"color"}       = "$color_scint";
 $detector{"type"}       = "Tube";
 $detector{"dimensions"} = "$Rmin*cm $Rmax*cm $Dz*cm $Sphi*deg $Dphi*deg";
...
sub make_ec_shield_scin3
{
 my $id = $_[0];
 my $Dz    = $Thickness_shield_scin/2;

 my %detector=init_det();
 $detector{"name"}        = "$DetectorName\_shield_scin3";
 $detector{"mother"}      = "$DetectorMother";
 $detector{"description"} = $detector{"name"};
 $detector{"pos"}         = "0*cm 0*cm $z_shield_scin3*cm";
 $detector{"rotation"}    = "0*deg 0*deg 0*deg";
 $detector{"color"}       = "$color_scint";
 $detector{"type"}       = "Tube";
 $detector{"dimensions"} = "$Rmin*cm $Rmax*cm $Dz*cm $Sphi*deg $Dphi*deg";
...
sub make_ec_module
{   
# build the module 1 x=-39.116cm y=-120.984cm first, at radius near 125cm, it exist in all three calorimeter, then make copy of it
# the original has to be made before copies, because GEMC make volume in alphabetic order of names, the origin name has to be before copies, one way to do it is to make it shorter
	  make_ec_module_shower(2);
          make_ec_module_supportsh(2);
	  make_ec_module_prescint(2);
	  make_ec_module_support(2);
...
{   
    ...
    if($id == 2){ $detector{"name"}        = "$DetectorName\_showe$id";}
    ...
    else {  $detector{"type"}       = "CopyOf $DetectorName\_showe2";}         
    $detector{"material"}   = "$material_scint";
    $detector{"mfield"}     = "no";
    $detector{"ncopy"}      = 1;
    $detector{"pMany"}       = 1;
...
{   
    ...
    if($id == 2){ $detector{"name"}        = "$DetectorName\_prescin$id";}
    ...
    else {  $detector{"type"}       = "CopyOf $DetectorName\_prescin2";}      
    $detector{"material"}   = "$material_scint";
    $detector{"mfield"}     = "no";
    $detector{"ncopy"}      = 1;
    $detector{"pMany"}       = 1;
...
sub make_ec_module_support()
{
    ...
    if($id == 2){ $detector{"name"}        = "$DetectorName\_suppor$id";}
   ...
   if($id == 2){
    ...
    else {  $detector{"type"}       = "CopyOf $DetectorName\_suppor2";}
...
sub make_ec_module_supportsh()
{
    my $id = $_[0];

    my $Dr = $Radius_shower;
    my $Dz_supportsh = $Thickness_supportsh/2;

    my %detector=init_det();

    if($id == 2){ $detector{"name"}        = "$DetectorName\_supporsh$id";}
    else {$detector{"name"}        = "$DetectorName\_supportsh$id";}
    $detector{"mother"}      = "$DetectorMother" ;
    $detector{"description"} = $detector{"name"};
    $detector{"pos"}        = "$x[$id-1]*cm $y[$id-1]*cm $z_supportsh*cm";
    $detector{"rotation"}   = "0*deg 0*deg $Angle_module*deg";
    $detector{"color"}      = $color_support;
   if($id == 2){
    $detector{"type"}       = "Pgon";
    $detector{"dimensions"} = "$Sphi*deg $Dphi*deg 6*counts 2*counts 0*cm 0*cm $Dr*cm $Dr*cm -$Dz_supportsh*cm $Dz_supportsh*cm";
    }
    else {  $detector{"type"}       = "CopyOf $DetectorName\_supporsh2";}
    $detector{"material"}   = "$material_support";
    $detector{"mfield"}     = "no";
    $detector{"ncopy"}      = 1;
    $detector{"pMany"}       = 1;
