...
// "background_solid_CLEO_SIDIS_He3_other_eDIS_1e6_output.root",
// "background_solid_CLEO_SIDIS_He3_other_eDIS_1e6_output.root"
// "background_solid_CLEO_SIDIS_He3_sum_actual_all_1e6_output.root",
// "background_solid_CLEO_SIDIS_He3_sum_actual_all_1e6_output.root",
// "background_solid_CLEO_SIDIS_He3_sum_actual_all_1e6_output.root",
// "background_solid_CLEO_SIDIS_He3_sum_actual_all_1e6_output.root",
// "background_solid_CLEO_SIDIS_He3_sum_actual_all_1e6_output.root"
...
int color[m]={1,2,3,4,5};
// char *hst[m]={"EdepR_Preshower_high","EdepR_Preshower_high","EdepR_Preshower_high","EdepR_Preshower_high","EdepR_Preshower_high"};
char *hst[m]={"EdepR_Preshower_low","EdepR_Preshower_low","EdepR_Preshower_low","EdepR_Preshower_low","EdepR_Preshower_low"};

// char *hst[m]={"EdepR_Preshower","EdepR_Preshower","EdepR_Preshower","EdepR_Preshower","EdepR_Preshower"};
//char *hst[m]={"EdepR_Shower","EdepR_Shower","EdepR_Shower","EdepR_Shower","EdepR_Shower"};
...
//bool Is_SIDIS=true,Is_PVDIS=false;
bool Is_SIDIS=false,Is_PVDIS=true;
...
//   h[j][i]->SetAxisRange(50,300);  
  ...
  if (Is_PVDIS){
    cout << "PVDIS" << endl;
    if (j==0) cout << " i " << i << " j " << j << " " << h[j][i]->GetBinContent(12) << " " << h[j][i]->GetBinContent(17) << " " << h[j][i]->GetBinContent(26) << endl;
  }
  else if (Is_SIDIS){
//     cout << "SIDIS" << endl;    
//     if (j==0) cout << " i " << i << " j " << j << " " << h[j][i]->GetBinContent(10) << " " << h[j][i]->GetBinContent(15) << " " << h[j][i]->GetBinContent(23) << endl;
//     if (j==1) cout << " i " << i << " j " << j << " " << h[j][i]->GetBinContent(9) << " " << h[j][i]->GetBinContent(11) << " " << h[j][i]->GetBinContent(14) << endl;

    if (j==0) cout << " i " << i << " j " << j << " " << h[j][i]->GetBinContent(10)/2.5 << " " << h[j][i]->GetBinContent(15)/2.5 << " " << h[j][i]->GetBinContent(23)/2.5 << endl;
    if (j==1) cout << " i " << i << " j " << j << " " << h[j][i]->GetBinContent(9)/2.5 << " " << h[j][i]->GetBinContent(11)/2.5 << " " << h[j][i]->GetBinContent(14)/2.5 << endl;
  }
...
//       cout << nbins << " " << binwidth << endl;
      double sum=0,number_module=0;
      for(int k=1;k<nbins;k++){      
	if (Is_PVDIS) {
	  if (h[j][i]->GetBinCenter(k) < 110 || 260< h[j][i]->GetBinCenter(k)) continue;	//PVDIS FAEC
	}
	else if (Is_SIDIS) {
	  if (j==0 && (h[j][i]->GetBinCenter(k) < 90 || 230< h[j][i]->GetBinCenter(k))) continue; //SIDIS FAEC
	  if (j==1 && (h[j][i]->GetBinCenter(k) < 80 || 140< h[j][i]->GetBinCenter(k))) continue; //SIDIS LAEC
	}
	
	if(Is_R) {
	  number_module += 2*3.1415926*h[j][i]->GetBinCenter(k)*binwidth/100.;	  
	  sum += 2*3.1415926*h[j][i]->GetBinCenter(k)*binwidth/100.*h[j][i]->GetBinContent(k);
	}
...
//       cout << j << "\t" << label[i] << "\t" << sum << endl;
      rate_total[j][i]=sum/number_module;
//       rate_total[j][i]=sum/1000.;      
      cout << " i " << i << " j " << j << " " << rate_total[j][i] << endl;
   }  
}      
...
//     if(i==3 || i>6) continue;
char content[500];
if(Is_R) sprintf(content,"%s \t\t %.3g",label[i],rate_total[j][i]);
