...
void GetCheck(TString input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
	...
	*/
	...
	const int VP_After  = 2210000; //virtual plane after LGC
	const double Z_LGC =96.0; //cm
	const int Z_Out = 305.0; //cm VP position
	...
	for(Int_t i=0;i<nselected;i++){
		...
		double x_C =0, y_C = 0, R_C =0., Px_C =0, Py_C =0, Pz_C =0, P_C = 0.;
		...
		for (Int_t j=0;j<flux_nfluxhit;j++) {
			...
			double x_EC = *(flux_x+j)/10. + Delta_Z * (*(flux_px+j))/(*(flux_pz+j));//cm
			double y_EC = *(flux_y+j)/10. + Delta_Z * (*(flux_py+j))/(*(flux_pz+j));//cm
			...
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==ID_Pick && fmom>=LGC_Threshold){//#Eelectrons going out 
				if(FirstOne1<1){
					Count_High+=1.0;
					x_C = x_EC; y_C = y_EC; R_C=R_EC; P_C = fmom;
					Px_C = (*(flux_px+j)); Py_C = (*(flux_py+j)); Pz_C = (*(flux_pz+j));
					cerr<<"=============================@"<<endl;
					cerr<<Form("++++In LGC: %6d--,px=%7.2f py=%7.2f pz=%7.2f, x=%7.2f, y=%7.2f, R=%f", 
							i,Px_C, Py_C, Pz_C, x_C, y_C, R_C)<<endl;
			...
			if((*(flux_pid+j)==Electron)&&*(flux_ID+j)==ID_Pick && fmom>=LGC_Threshold){//#Eelectrons going out 
				//Count_Em++;
				if(FirstOne2<1){
					Count_Cut_Em+=EC_Cut;
				}
		...
		if(FirstOne1>0){
			...
			//	if(fmom<8.e-1) continue;

				int ID_Pick = 3110000;
				if((abs(*(flux_pid+j))==Electron)&&*(flux_ID+j)==ID_Pick){//#Eelectrons going out
					//if(abs(*(flux_px+j)-Px_C)<20.0 &&abs(*(flux_py+j)-Py_C)<20.0 &&abs(*(flux_pz+j)-Pz_C)<20.0){  
					if(abs(fmom-P_C)<0.01){
							cerr<<Form("====In  EC: %6d--,px=%7.2f py=%7.2f pz=%7.2f, x=%7.2f, y=%7.2f, R=%f", 
									i,(*(flux_px+j)),(*(flux_py+j)),(*(flux_pz+j)),(*(flux_x+j))/10.,(*(flux_y+j))/10.,r)<<endl;
							cerr<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"<<endl;

						if(abs(*(flux_x+j)-x_C*10.)<20.0 && abs(*(flux_y+j)-y_C*10.)<20.0){
					//		cerr<<Form("++++In  EC: %6d--,px=%7.2f py=%7.2f pz=%7.2f, x=%7.2f, y=%7.2f", 
					//				i,(*(flux_px+j)),(*(flux_py+j)),(*(flux_pz+j)),(*(flux_x+j))/10.,(*(flux_y+j))/10.)<<endl;
