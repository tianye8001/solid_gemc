		...
		if(*gen_theta>=5.0){
			...
			for (Int_t j=0;j<flux_nfluxhit;j++) {
				if(*(flux_ID+j)==ID_Pick){//
					...
					if(abs(*(flux_pid+j))==Electron && fmom<EC_Threshold){//#Eelectrons going out 
						if(FirstOne0<1)
							Count_Low+=1.0;
						FirstOne0 ++;;
					}
					...
					//Count by slides
					if(*(flux_pid+j)==Gamma&&EC_Cut>=0.1){//#Photons going in
						if(FirstOne4<1)
							EC_In_G[Module_ID][Slide_ID]+=EC_Cut_Max;
						FirstOne4 ++;;
					}
					if(abs(*(flux_pid+j))==Electron&&EC_Cut>=0.1){//#Eelectrons going out 
						if(FirstOne5<1){
					...
					for (Int_t j=0;j<flux_nfluxhit;j++) {
						...
						if((*(flux_pid+j)==Electron)&&*(flux_ID+j)==ID_Pick){//#Eelectrons going out 
							...
							if(EC_Cut>=0.9){
								cerr<<"==============================================="<<endl;
								cout <<i<<"  "<< j << endl;
								cout <<flux_evn<<" "<<*(flux_ID+j)<<" "<< *(flux_pid+j)<<" "<<*(flux_mpid+j)<<" " 
			...
			for (Int_t j=0;j<flux_nfluxhit;j++) {
				if(*(flux_ID+j)==ID_Pick ){
					...
					if(*(flux_pid+j)==Gamma&& EC_Cut_Max>=0.1){//#Photons going out 
