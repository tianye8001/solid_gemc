...
int main()
{
	...
	TString ROOT_Dir = "/volatile/halla/solid/yez/sidis/gemc2/3he_pip_11/";
	/*Set Branch{{{*/
/*    TString input_filename = "SIDIS_3he_pip_11_0.root";
    //TString input_filename = "SIDIS_3He.root";
...
*/
	...
	const double LAEC_VP_Rmin = 75.0, LAEC_VP_Rmax = 144.0, LAEC_VP_Z = -67.0;//cm
	const int FAEC_VP_ID = 3110000;
	const double FAEC_VP_Rmin = 90.0, FAEC_VP_Rmax = 265.0, FAEC_VP_Z = 413.0;//cm
...
//					)<<endl;
       ...
       /*}}}End Fill EC*/
		...
		if(Is_LAEC_Hit&&Count_GEM_Hit_LA>=3){
            LA_evn ++;
...
//						GEM_Mom[ip],GEM_Theta[ip],GEM_Phi[ip],
		...
		if(Is_FAEC_Hit&&Count_GEM_Hit_LA>=2&&Count_GEM_Hit_FA>=1){
            FA_evn ++;
