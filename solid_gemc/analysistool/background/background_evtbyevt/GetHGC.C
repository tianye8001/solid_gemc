...
void GetHGC(TString input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
...
/*
	...
	*/
	...
	double HGC_Out_G[HGC_Module][HGC_Slide];// Number of photons going into the device
	double HGC_Out_E[HGC_Module][HGC_Slide];// Number of electrons going into the device

	for(int i=0;i<HGC_Slide;i++){//I just want to initialize everything
		HGC_R[i] = trig_cut_range_R[i]; //{0,105,115,130,150,200,300};
		for(int j=0;j<HGC_Module;j++){
			HGC_In_G[j][i] =0.;
			HGC_In_E[j][i] =0.;
			HGC_Out_G[j][i] =0.;
			HGC_Out_E[j][i] =0.;
	...
	for(Int_t i=0;i<nselected;i++){
		...
		for (Int_t j=0;j<flux_nfluxhit;j++) {
			...
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==ID_Pick && fmom<HGC_Threshold){//#Eelectrons going out 
				if(FirstOne0<1)
					Count_Low+=1.0;
				FirstOne0 ++;;
			}
			...
			if((*(flux_pid+j)==Electron)&&*(flux_ID+j)==ID_Pick && fmom>=HGC_Threshold){//#Eelectrons going out 
				//Count_Em++;
				if(FirstOne2<1)
					Count_Cut_Em+=EC_Cut;
				FirstOne2 ++;;
            ...
            //Count by slides
			if(*(flux_pid+j)==Gamma&&*(flux_ID+j)==ID_Pick && fmom>=HGC_Threshold)//#Photons going in
		...
		for (Int_t j=0;j<flux_nfluxhit;j++) {
			...
			if(*(flux_pid+j)==Gamma&&*(flux_ID+j)==ID_Pick && fmom>=HGC_Threshold ){//#Photons going out 
	...
	ofstream outfile(Form("HGC_%s",input_out.Data()));
	for(int k=0;k<HGC_Slide;k++){
		for(int l=0;l<HGC_Module;l++){
			 In_G +=HGC_In_G[l][k];
			 In_E +=HGC_In_E[l][k];
			 Out_E +=HGC_Out_E[l][k];
			 Out_G +=HGC_Out_G[l][k];
