...
void GetTest(TString input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
...
/*
	...
	*/
	...
	const double R_Min = 58.0;//cm, VP position, LGC front = 65.1, back = 85.1 
	const double R_Max = 127.0;//cm, VP position, LGC front=143.9 LGC back 264.9cm .
	...
	const int VP_Before = 2110000; //Light GC virtual plane
	const int VP_After  = 2210000; //Heavy GC virtual plane
    const double Z_EC = 425.5; //cm EC front
	const double Z_LGC = 96.0; //cm VP position, 194.0025 is the real positon of LGC
	...
	double Count_High = 0, Count_Low = 0, Count_Cut =0;
	double EC_Cut = 0.0;
	cerr<<"++++++++++++++++ "<<endl;
	for(Int_t i=0;i<nselected;i++){
		...
		for (Int_t j=0;j<flux_nfluxhit;j++) {
			...
			R_EC = abs((Z_EC - Z_LGC) * tan(vertex_theta/DEG) + r);
			if(R_EC>230.0 || R_EC<96.00) continue;
			...
			if(EC_Cut<-1e-9||EC_Cut>1){
				cerr<<"---- I can't find the cut!"<<Form(" --- Theta=%f, R= %f,  E =%f",vertex_theta, R_EC, fmom)<<endl;
			   	return;
			}
			...
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==ID_Pick && fmom>=2){//#Eelectrons going out 
				Count_Cut+=EC_Cut;
				cerr<<"==============================================="<<endl;
				cout <<i<<"  "<< j << endl;
				cout << flux_evn<< " " << *(flux_ID+j) << " " << *(flux_pid+j) << " " << *(flux_mpid+j) << " " << *(flux_E+j) << " " << " " << *(flux_vx+j) << " " << *(flux_vy+j) << " " << *(flux_vz+j)  << " " << *(flux_px+j) << " " << *(flux_py+j) << " " << *(flux_pz+j) <<" R="<<R_EC << endl;
				cerr<<"o================================"<<endl;

				bGood=kTRUE;
			} 

			if(bGood&&(*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==3110000 && fmom>=2){//#Eelectrons going out 
				r=sqrt(pow(*(flux_x+j),2)+pow(*(flux_y+j),2))/10.;//cm
				cerr<<"************************************************"<<endl;
				cout <<i<<"  "<< j << endl;
				cout << flux_evn<< " " << *(flux_ID+j) << " " << *(flux_pid+j) << " " << *(flux_mpid+j) << " " << *(flux_E+j) << " " << " " << *(flux_vx+j) << " " << *(flux_vy+j) << " " << *(flux_vz+j)  << " " << *(flux_px+j) << " " << *(flux_py+j) << " " << *(flux_pz+j) <<" R="<<r<< endl;
				cerr<<"*********************************"<<endl;
			}

			if(*(flux_pid+j)==Gamma&&*(flux_ID+j)==VP_Before && fmom>=LGC_Threshold)//#Photons going in
				LGC_In_G[Module_ID][Slide_ID]+=EC_Cut;
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==VP_Before && fmom>=LGC_Threshold){//#Eelectrons going out 
				LGC_In_E[Module_ID][Slide_ID]+=EC_Cut;
			}
			if(*(flux_pid+j)==Gamma&&*(flux_ID+j)==VP_After && fmom>=LGC_Threshold)//#Photons going out 
				LGC_Out_G[Module_ID][Slide_ID]+=EC_Cut;
			if((*(flux_pid+j)==Electron||*(flux_pid+j)==-Electron)&&*(flux_ID+j)==VP_After && fmom>=LGC_Threshold)//#Eelectrons going out 
	...
	cerr<< Form("&&&& N (P<1GeV) = %d, N (P>1.0GeV) = %d, after Cut N = %d / %d", (int)(Count_Low), (int)(Count_High), (int)(Count_Cut), (int)(In_E))<<endl;
