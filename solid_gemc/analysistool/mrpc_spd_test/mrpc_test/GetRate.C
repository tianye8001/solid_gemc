...
void GetRate(TString input_filename)
{
	...
	TString input_txt = input_filename;
	input_txt.ReplaceAll(".root","g2e.txt");
    ofstream outfile(input_txt.Data());
	...
	for(Int_t i=0;i<nevent;i++){
		...
		for(int k=0;k<MRPC_Gas_Layer;k++)
			Counter[k] = 0;
        Counter_All = 0;

		for (Int_t j=0;j<flux_nfluxhit;j++) {
		//	if (*(flux_ID+j)>=4000000){  //MRPC
			//if ((*(flux_ID+j)>=4100000&&*(flux_ID+j)<=4120010)  //GAS&&Glass
			//||  (*(flux_ID+j)>=4120000&&*(flux_ID+j)<=4120020)){ 
			if(*(flux_ID+j)>=4100000&&*(flux_ID+j)<=4120100) { //GAS
				
				if(*(flux_pid+j)==Gamma) continue; //MRPC doesn't sense photon
				...
				for(int k=0;k<MRPC_Gas_Layer;k++){
				//	if(abs(z-Gas[k])<(MRPC_Gas_Thick+MRPC_Glass_Thick)/2.0) //Gas+Glass
			//		if(abs(z-Gas[k])<(MRPC_Gas_Thick+MRPC_Glass_Thick)/2.0&&abs(z-Gas[k])>MRPC_Gas_Thick/2.0)//Glass Only
					if(abs(z-Gas[k])<MRPC_Gas_Thick/2.0) //Gas Only
						Counter[k]++;
				}
		...
		for(int k=0;k<MRPC_Gas_Layer;k++){
			Counter_All += Counter[k];
		}
		if(Counter_All>=2)
			G_2_E ++;
	//	if(input_filename.Contains("mrpc_e_"))
	//		Counter_All -= 10;
	}
	/*End Read in each event}}}*/

		double R =G_2_E*1.0/(nevent*1.0)*100.0;
	    double R_Err = R*sqrt(1.0/G_2_E+1.0/nevent);
	    outfile << 	input_filename.Data() << "   "<< R << "  "<< R_Err<<endl;
		cerr<<" --- Gamma_2_E = "<<G_2_E<<" %= "<< R<<"%, "<<" +/-"<<R_Err<<"%"<<endl;
