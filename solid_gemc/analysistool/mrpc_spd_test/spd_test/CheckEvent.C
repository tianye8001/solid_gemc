...
void CheckEvent(string input_filename)
{
	...
	int Just_One = 0;
	cout <<"flux evn#"<<"  flux# " << "  fluxID"<<"  fluxPID"<<"  fluxMPID"<<"  fluxEDep"<<"  fluxE"<<" ....."<<endl;
	for(Int_t i=0;i<nevent;i++){
		cout<<i<<"\r";
		Tflux->GetEntry(i);

		if(flux_nfluxhit>=6&&Just_One==0){
...
//				if(*(flux_ID+j)>=5100000&&*(flux_ID+j)<=4120000) { //SPD
				...
				//	if(*(flux_pid+j)==Gamma) continue; //SPD doesn't sense photon
					//if(*(flux_pid+j)==Gamma&&*(flux_mpid+j)==Beam) continue; //not the orginal photon
	//				if(*(flux_mpid+j)==Beam) continue; //SPD doesn't sense photon
					...
					if(1){
						...
						Just_One=1;
						cout << j << endl;
						cout <<Form("%3d %2d %7d %3d %3d %3.2e %3.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e",
								flux_evn,flux_nfluxhit,*(flux_ID+j),*(flux_pid+j),*(flux_mpid+j),
								*(flux_Edep+j),*(flux_E+j),*(flux_x+j),*(flux_y+j),*(flux_z+j),
								*(flux_vx+j),*(flux_vy+j),*(flux_vz+j),*(flux_px+j),*(flux_py+j),*(flux_pz+j) 
								)<<endl;
					}
