...
void GetResponse(TString input_filename, TString particle)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
	...
	if (file->IsZombie()) {
		cout << "Error opening file" << input_filename.Data() << endl;
		exit(-1);
	}
	else cout << "open file " << input_filename.Data() << endl;
	...
	const int Proton = 11;
	const int Neutron = 11;
	const int Gamma = 22;
	const int Pim =-211;
	const int Pip = 211;
	double fmom = -1000.1, r = -1000.1, z = -1000.0;//cm
    double EDep = 0.0, Ein = 0.0, EDep_E = 0.0;
	int PID = 0;
	if(particle=="e-")
        PID = Electron;
	else if(particle=="e+")
		...
		cerr<<"*** ERROR, I don't know the particle!"<<endl;
	
	TH2D *H_EDep = new TH2D("H_EDep",Form("%s Energy Response",particle.Data()),500, 0.0, 200.,500,-6, 1.3);
	H_EDep->SetXTitle("SPD Energy Deposition (MeV)");
	H_EDep->GetXaxis()->CenterTitle(1);
	H_EDep->GetXaxis()->SetTitleFont(32);
	H_EDep->SetYTitle("Momentum of Incoming Particle (GeV)");
	H_EDep->GetYaxis()->CenterTitle(1);
	H_EDep->GetYaxis()->SetTitleFont(32);
	TH2D *H_EDep_Re = new TH2D("H_EDep_Re",Form("%s Energy Response",particle.Data()),500, 0.0, 1.0,500,-6, 1.3);
   	H_EDep_Re->SetXTitle("SPD Energy Deposition / Momentum");
	H_EDep_Re->GetXaxis()->CenterTitle(1);
	H_EDep_Re->GetXaxis()->SetTitleFont(32);
	H_EDep_Re->SetYTitle("Momentum of Incoming Particle (GeV)");
	H_EDep_Re->GetYaxis()->CenterTitle(1);
	H_EDep_Re->GetYaxis()->SetTitleFont(32);
	 /*}}}*/

	for(Int_t i=0;i<nevent;i++){
		EDep = 0.0; Ein = 0.0; EDep_E = 0.0;
		...
		//	if (*(flux_ID+j)==5000000){  //SPD&virtual plane
			if (*(flux_ID+j)==5100000||*(flux_ID+j)==5110000){  //SPD
		//	if (*(flux_ID+j)==5110000){  //virtual plane
				
				//if(*(flux_pid+j)==PID) continue; //SPD doesn't sense photon
				...
				EDep += *(flux_Edep+j); //MeV
			} //Gas & Electron & r>96,r<210
		}//Flux particles in one event

			if(EDep<1e-3)
				cerr<<Form(" --- Ein = %e, EDep = %e", Ein*1000.,EDep )<<endl;


		//	cerr<<Form(" --- E0 = %10.4f, EDep = %10.4f",Ein, EDep)<<endl;

			H_EDep->Fill(EDep, log10(Ein));
			H_EDep_Re->Fill(EDep/(Ein*1000.), log10(Ein));
	}
	/*End Read in each event}}}*/

	for ( int x = 1; x <= EDepPreshower_Dist->GetNbinsX(); x++ )
	  {
            EDepPreshower_Dist->SetBinContent(
					      x,
					      y,
					      EDepPreshower_Dist->GetBinContent(x, y)
					      / EDepPercent_Ebin->GetBinContent(y));
	  }
        for ( int x = 1; x <= EDepShowerOverE_Dist->GetNbinsX(); x++ )
	  {
            EDepShowerOverE_Dist->SetBinContent(
						x,
						y,
						EDepShowerOverE_Dist->GetBinContent(x, y)
						/ EDepPercent_Ebin->GetBinContent(y));
	  }
	
	TCanvas *c1 = new TCanvas("c1","c1",1200,600);
	c1->Divide(2,1);
	c1->cd(1); H_EDep->Draw("colz");
	c1->cd(2); H_EDep_Re->Draw("colz");
	
	
	TString output_filename = input_filename;
	output_filename.ReplaceAll(".root","_histnew.root");
	TFile *outf = new TFile(output_filename.Data(),"recreate");
	H_EDep->Write(); H_EDep_Re->Write(); outf->Close();
