...
static uRgrooveConstants initializeuRgrooveConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", bool accountForHardwareStatus = false)
{
	// all these constants should be read from CCDB
	uRgrooveConstants urgrooveC;
	
	// do not initialize at the beginning, only after the end of the first event,
	// with the proper run number coming from options or run table
	if(runno == -1) return urgrooveC;
	string timestamp = "";
	...
	// database
	urgrooveC.runNo = runno;
	urgrooveC.date       = "2022-08-23";
	if(getenv ("CCDB_CONNECTION") != nullptr)
		urgrooveC.connection = (string) getenv("CCDB_CONNECTION");
	else
		urgrooveC.connection = "mysql://clas12reader@clasdb.jlab.org/clas12";
	...
	/*
	 ...
	 sprintf(fmtc.database,"/geometry/uRgroove/uRgroove_global:%d:%s%s", fmtc.runNo, digiVariation.c_str(), timestamp.c_str());
	...
	// uRGroove groove top width: 70 um = 0.07 mm
	urgrooveC.stripU_width[0] = 0.07;
	urgrooveC.stripU_width[1] = 0.07;
	urgrooveC.stripU_width[2] = 0.07;
	urgrooveC.stripU_width[3] = 0.07;

	urgrooveC.stripU_width_proto[0] = 0.07;
	urgrooveC.stripU_width_proto[1] = 0.07;
	urgrooveC.stripU_width_proto[2] = 0.07;
	urgrooveC.stripU_width_proto[3] = 0.07;

	urgrooveC.stripV_stereo_angle = 10 ; // angle between strip and trapezoid base in degree
	urgrooveC.stripV_pitch = 0.2;  // mm, uRGroove readout pitch: 200 um

	urgrooveC.stripV_width[0] = 0.07;
	urgrooveC.stripV_width[1] = 0.07;
	urgrooveC.stripV_width[2] = 0.07;
	urgrooveC.stripV_width[3] = 0.07;

	urgrooveC.stripV_width_proto[0] = 0.07;
	urgrooveC.stripV_width_proto[1] = 0.07;
	urgrooveC.stripV_width_proto[2] = 0.07;
	urgrooveC.stripV_width_proto[3] = 0.07;
	
	urgrooveC.w_i=25; //ionization potential assumed to be 25 eV
	urgrooveC.sigma_td= 0.5;         // effective value to take into account transverse diffusion + charge dispersion
	urgrooveC.nb_sigma = 5;            // Number of sigma to study around the closest strip
	urgrooveC.gain =1E4;
	
	// drift velocity
	urgrooveC.v_drift = 5E-3; // velocity drift [cm/ns]
	urgrooveC.sigma_time = 20; // time resolution 20 ns
...
map<string, double>uRgroove_HitProcess :: integrateDgt(MHit* aHit, int hitn)
{
    ...
    if(identity[4].id ==-15000){
    	...
    	dgtz["ADC_ADC"]  = (1.0*(int) (uRgrooveC.gain*1e6*tInfos.eTot/uRgrooveC.w_i));
...
vector<identifier> uRgroove_HitProcess::processID(vector<identifier> id, G4Step* aStep, detector Detector)
{
    vector<identifier> yid;
    uRgroove_strip URgroove_strip;
    ...
    // ------------------------------------------------------------
    uRgrooveC.get_strip_info("strip_u", isProto);
    vector<uRgroove_strip_found> multi_hit_u =
    ...
    /*cout << "  U strip " << h
        ...
        for (unsigned int j = 0; j < id.size(); j++) {
            ...
            this_id.id_sharing =
                multi_hit_u.at(h).weight / (uRgrooveC.gain * 1e6 * depe / uRgrooveC.w_i);
    ...
    // ------------------------------------------------------------
    uRgrooveC.get_strip_info("strip_v", isProto);
...
cout<<endl;*/
   ...
   /* cout << "  V strip " << h
        ...
        for (unsigned int j = 0; j < id.size(); j++) {
            ...
            this_id.id_sharing =
                multi_hit_v.at(h).weight / (uRgrooveC.gain * 1e6 * depe / uRgrooveC.w_i);
...
/*vector<identifier> uRgroove_HitProcess :: processID(vector<identifier> id, G4Step* aStep, detector Detector)
{
	...
	// STRIP U 
    uRgrooveC.get_strip_info("strip_u", isProto);
    vector<uRgroove_strip_found> multi_hit_u = URgroove_strip.FindStrip(lxyz, depe, uRgrooveC, time, isProto);
    ...
    for(int h=0; h<n_multi_hits_u; h++){
    		...
    		for(int j=0; j<5; j++)
    		{
    			...
    			if(j==4){    //J==4 strip ID
    				if(id[2].id>0) {
					    if(multi_hit_u.at(h).numberID ==-15000){
					    	...
					    	this_id.id  = multi_hit_u.at(h).numberID + std::accumulate(uRgrooveC.number_strip_chamber,uRgrooveC.number_strip_chamber +id[2].id-1,0);
					    }
    			...
    			this_id.TimeWindow = id[j].TimeWindow;
    			this_id.TrackId    = id[j].TrackId;
    			this_id.id_sharing = multi_hit_u.at(h).weight/(uRgrooveC.gain*1e6*depe/uRgrooveC.w_i);
    ...
    // STRIP V 
    uRgrooveC.get_strip_info("strip_v", isProto);
	vector<uRgroove_strip_found> multi_hit_v = URgroove_strip.FindStrip(lxyz, depe, uRgrooveC, time, isProto);
	int n_multi_hits_v = multi_hit_v.size();
	 ...
	 for(int h=0; h<n_multi_hits_v; h++){
	 		for(int j=0; j<5; j++)
	 		{
				...
				if(j==4){    //J==4 strip ID
				   if(id[2].id>0) {
					    if(multi_hit_v.at(h).numberID ==-15000){
					    	...
					    	this_id.id  = multi_hit_v.at(h).numberID + std::accumulate(uRgrooveC.number_strip_chamber,uRgrooveC.number_strip_chamber +id[2].id-1,0);
					    }
				...
				this_id.TimeWindow = id[j].TimeWindow;
				this_id.TrackId    = id[j].TrackId;
				this_id.id_sharing = multi_hit_v.at(h).weight/(uRgrooveC.gain*1e6*depe/uRgrooveC.w_i);
				yid.push_back(this_id);
	 	}
	 		}
...
vector<MHit*> uRgroove_HitProcess :: electronicNoise()
{
...
map< int, vector <double> > uRgroove_HitProcess :: chargeTime(MHit* aHit, int hitn)
{
...
double uRgroove_HitProcess :: voltage(double charge, double time, double forTime)
{
	return 0.0;
}

map< string, vector <int> >  uRgroove_HitProcess :: multiDgt(MHit* aHit, int hitn)
{
...
void uRgroove_HitProcess::initWithRunNumber(int runno)
{
	string digiVariation    = gemcOpt.optMap["DIGITIZATION_VARIATION"].args;
	string digiSnapshotTime = gemcOpt.optMap["DIGITIZATION_TIMESTAMP"].args;
	
	if(uRgrooveC.runNo != runno) {
		cout << " > Initializing " << HCname << " digitization for run number " << runno << endl;
		uRgrooveC = initializeuRgrooveConstants(runno, digiVariation, digiSnapshotTime, accountForHardwareStatus);
		uRgrooveC.runNo = runno;
	}
}
...
uRgrooveConstants uRgroove_HitProcess::uRgrooveC = initializeuRgrooveConstants(1);
