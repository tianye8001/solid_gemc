#ifndef uRgroove_strip_H
#define uRgroove_strip_H 1
...
public:
	...
	vector<uRgroove_strip_found> FindStrip( G4ThreeVector   xyz , double Edep, uRgrooveConstants uRgroovec, double time, bool isProto); // Strip Finding Routine
	void Carac_strip(int strip, uRgrooveConstants uRgroovec); //length of the strip
	double Weight_td(int strip, double x, double y, double z, uRgrooveConstants uRgroovec, bool isProto); //Compute the fraction of Nel falling onto the strip, depending on x,y in the FMT coordinate system
	bool Build_strip(int strip, uRgrooveConstants uRgroovec );
	...
	G4ThreeVector change_of_coordinates(G4ThreeVector   xyz , uRgrooveConstants uRgroovec);
	
	double GetBinomial(double n, double p);
	int Number_of_strip(uRgrooveConstants uRgroovec);
	int strip_id(int i, uRgrooveConstants uRgroovec);
