...
void GetEC(TString input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
	...
	const double Z_EC = 415.0; //cm, EC front, VP position = 413cm
	const double Z_VP = 413.0; //cm VP position
	const double R_Min_Out = 105.0;//cm, EC front
	const double R_Max_Out = 235.0;//cm,
	const double Z_Out = 465.0; //cm VP position
	...
	for(Int_t i=0;i<nselected;i++){
		...
		if(gen_theta[Is_Electron]>=5.0){
			...
			/*Count particles into the device{{{*/
			...
			for (unsigned int j=0;j<flux_hitn->size();j++) {
				...
				if(ID_flux==ID_Pick){//
					...
					if(r > R_Max||r<R_Min) continue;//The radius of a EC sector is 210cm;
					...
					double vertex_theta = acos((flux_pz->at(j))/sqrt(pow(flux_px->at(j),2)+pow(flux_py->at(j),2)+pow(flux_pz->at(j),2)))*DEG;
					//R_EC = abs((Z_EC - Z_VP) * tan(vertex_theta/DEG) + r);
					R_EC = r; //VP is very close to EC
			...
			for (unsigned int j=0;j<flux_hitn->size();j++) {
				...
				if(ID_flux==ID_Pick){//
					...
					if(abs(PID_flux)==Electron && fmom<EC_Threshold){//#Electrons going out 
						if(FirstOne0<1)
							Count_Low+=1.0;
						FirstOne0 ++;;
					}
					...
					if((PID_flux==Electron)&&EC_Cut_Max>=0.1){//#Electrons going out 
						if(FirstOne2<1)
					...
					//Count by slides
					if(PID_flux==Gamma&&EC_Cut>=0.1){//#Photons going in
						if(FirstOne4<1)
							EC_In_G[Module_ID][Slide_ID]+=EC_Cut_Max;
						FirstOne4 ++;;
					}
					if(abs(PID_flux)==Electron&&EC_Cut>=0.1){//#Electrons going out 
						if(FirstOne5<1){
				...
				if(bPrint){
					...
					for (unsigned int j=0;j<flux_hitn->size();j++) {
						...
						if((PID_flux==Electron)&&ID_flux==ID_Pick){//#Eelectrons going out 
							//if(fmom>=EC_Threshold*0.9){
							if(EC_Cut>=0.9){
								cerr<<"==============================================="<<endl;
								cout <<i<<"  "<< j << endl;
								cout <<ID_flux<<" "<< PID_flux<<" "<<flux_mpid->at(j)<<" " 
									<<" Mom="<<fmom <<" Theta="<<vertex_theta<<" R="<<r<<", Cut="<<EC_Cut<< endl;
								cerr<<"================================="<<endl;
							}
						}
						//High Energy Positron with EC R-Cut 
			...
			/*Count particles out from the device{{{*/
			FirstOne6 = 0;
			ID_Pick = VP_After;
			EC_Cut_Max = 0.0;
			for (unsigned int j=0;j<flux_hitn->size();j++) {
			...
			for (unsigned int j=0;j<flux_hitn->size();j++) {
				...
				if(ID_flux==ID_Pick ){
					...
					if(PID_flux==Gamma&& EC_Cut_Max>=0.1){//#Photons going out 
