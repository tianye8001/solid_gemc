...
void GetFASPD(TString input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
    ...
    const int VP_Before = 5110000; //SPD virtual plane
	const int VP_After  = 4110000; //MRPC virtual plane
	const double Z_SPD = 406.5; //cm VP position, 407.5 is the real positon of SPD
	const double R_Min = 96.0;//cm, VP position, SPD 
	const double R_Max = 210.0;//cm, VP position, SPD
    const double Z_Out = 408.0; //cm MRPC VP position
	const double R_Min_Out = 96.0;//cm, MRPC front
	const double R_Max_Out = 210.0;//cm,
	...
	for(Int_t i=0;i<nselected;i++){
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if(PID_flux==Gamma&&ID_flux==ID_Pick && fmom>=0.95*EC_Threshold ){//#Photons going out 
				SPD_Out_G[Module_ID][Slide_ID]+=EC_Cut_Max;
			}
			if((PID_flux==Electron||PID_flux==-Electron)&&ID_flux==ID_Pick && fmom>=0.95*EC_Threshold){//#Eelectrons going out 
				Count_Both++;
				if(FirstOne4<1)
					SPD_Out_E[Module_ID][Slide_ID]+=EC_Cut_Max;
				FirstOne4 ++;;
			}
