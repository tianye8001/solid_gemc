...
void GetHGC(string input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
	...
	/*Define HGC{{{*/
    TString Detector_Name = "HGC";   
	const int VP_In = 2210000; //Front virtual plane
	const double Z_In = 305.0; //cm Front VP position
	const double R_Min_In = 83.1;//cm, Front VP position
	const double R_Max_In = 230.0;//cm, Front VP position
	
	const int VP_Out  = 5110000; //Rear virtual plane
	const double Z_Out = 406.5; //cm Rear VP position
