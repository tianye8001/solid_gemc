...
void GetLGC(string input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
	...
	/*Define LGC{{{*/
    TString Detector_Name = "LGC";   
	const int VP_In = 2110000; //Front virtual plane
	const double Z_In = 96.0; //cm Front VP position
	const double R_Min_In = 58.1;//cm, Front VP position
	const double R_Max_In = 126.9;//cm, Front VP position

	//Use HGC's front VP as the rear VP of LGC	
	const int VP_Out  = 2210000; //Rear virtual plane
	const double Z_Out = 305.0; //cm Rear VP position
	const double R_Min_Out = 83.0;//cm, Rear VP position
	const double R_Max_Out = 235.0;//cm, Rear VP position
       ...
       //    cerr<<Form("---#%d@%d Px=%f Py=%f Pz=%f P=%f Theta =%f Phi=%f ",i, ig,gen_px->at(ig),gen_py->at(ig),gen_pz->at(ig),gen_mom[ig],gen_theta[ig],gen_phi[ig])<<endl;
		...
		if(1){
			...
			for (unsigned int j=0;j<flux_hitn->size();j++) {
			    ...
			    //if(PID_flux==Gamma&&mom_flux>1.0)	
				...
				//	cerr<<"++++ EC_Cut_Max = "<<EC_Cut_Max<<endl;
				}
                if(EC_Cut_Max<1e-9) continue; 
					/*}}}*/
