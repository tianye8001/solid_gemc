...
void GetMRPC(string input_filename)
{
	gROOT->Reset();
	gStyle->SetPalette(1);
	gStyle->SetOptStat(0);
	...
	/*Define MRPC{{{*/
    TString Detector_Name = "MRPC";   
	const int VP_In = 4110000; //Front virtual plane
	const double Z_In = 408.0; //cm Front VP position
	const double R_Min_In = 96.0;//cm, Front VP position
	const double R_Max_In = 210.0;//cm, Front VP position
	
	const int VP_Out  = 3110000; //Rear virtual plane
	const double Z_Out = 413.0; //cm Rear VP position
	const double R_Min_Out = 105.0;//cm, Rear VP position
