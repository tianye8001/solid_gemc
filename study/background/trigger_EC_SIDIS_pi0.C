...
void trigger_EC_SIDIS_pi0()
{
	...
	int region_index=-1;
	cerr<<" --- Region Index = (0->SIDIS_FAEC, 1->SIDIS_LAEC)"; cin >> region_index;
    char* region;
	if(region_index==0) region="SIDIS_FAEC" ;
	else if (region_index==1) region="SIDIS_LAEC";
	...
	int pid[m]={1,1,1};
	char *label[m]={"#gamma(#pi^{0}(DIS))", "#gamma(#pi^{0}(DIS,up))", "#gamma(#pi^{0}(DIS,down))"};
	double mass[m]={0.,0.,0.};
	int color[m]={2,4,6};
	...
	const int n=1;
	TCanvas *c_Plog_R_ec = new TCanvas("Plog_R_ec","Plog_R_ec",n*800,1800);
	c_Plog_R_ec->Divide(2*n,m);
//	TCanvas *c_Plog_R_ec_trig = new TCanvas("Plog_R_ec_trig","Plog_R_ec_trig",n*400,1800);
//	c_Plog_R_ec_trig->Divide(n,m);
	...
	for(int i=0;i<m;i++){
		...
		for(int j=0;j<n;j++){
			...
			c_Plog_R_ec->cd(i*2*n+j+1);
			gPad->SetLogz(1);
			if (i!=3) hPlog_R[j][i]->SetAxisRange(-3,1.1,"Y");  
			hPlog_R[j][i]->Draw("colz");
			sprintf(hstname,"%s_%i_%i","Plog_R",det[region_index],pid[i]);  
			t1.DrawLatex(20, 1.15, Form("%s (%s)",label[i],region));
			...
			for(int k=1;k<=hPlog_R[j][i]->GetNbinsX();k++){
				...
				c_Plog_R_ec->cd(i*2*n+j+2);
				gPad->SetLogz(1);
				if (i!=3) hPlog_R_trig[j][i]->SetAxisRange(-3,1.1,"Y");    
				hPlog_R_trig[j][i]->Draw("colz"); 
				t1.DrawLatex(20, 1.15, Form("%s (%s) triggered",label[i],region));
			...
			for(int j=0;j<n;j++){
				c_fluxR_ec_proj->cd(j+1);
				{
					TPaveText *pt1 = new TPaveText(0.63,0.95-m*0.05,0.99,0.99,"brNDC");
					// TPaveLabel *pt1 = new TPaveLabel(0.7,0.5,0.95,0.95,"a \n b \r sdfhdfghdsfkghdkfghdshgh","brNDC");
					pt1->SetFillColor(19);
					pt1->SetTextAlign(12);
					pt1->Draw();
					TText *text0=pt1->AddText(Form("%s:",region));
				...
				// c_fluxR_ec_trig->cd(j+1)o
				{
					TPaveText *pt1 = new TPaveText(0.63,0.65-m*0.05,0.99,0.70,"brNDC");
					// TPaveLabel *pt1 = new TPaveLabel(0.7,0.5,0.95,0.95,"a \n b \r sdfhdfghdsfkghdkfghdshgh","brNDC");
					pt1->SetFillColor(19);
					pt1->SetTextAlign(12);
					pt1->Draw();
					TText *text0=pt1->AddText(Form("%s (triggerred):",region));
					text0->SetTextColor(1);
					text0->SetTextSize(0.035);
						for(int i=0;i<m;i++){
						//     if(i==3 || i>6) continue;
				...
				char *pic_name[2][3]={
					"Plog_R_FAEC_pi0.png",
					"Plog_R_FAEC_trig_pi0.png",
					"fluxR_FAEC_pi0.png",

					"Plog_R_LAEC_pi0.png",
					"Plog_R_LAEC_trig_pi0.png",
					"fluxR_LAEC_pi0.png",
				};
