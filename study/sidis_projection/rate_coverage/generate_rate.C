...
int main(){
	...
	int energy; cerr<<"--- Energy (8->8.8GeV,11->11GeV) = "; cin >>energy;
	...
	TChain *T=new TChain("T");
     double HADRON_CUT = 0.0;
	if(particle=="pion"){
		HADRON_CUT = 10.0;
		if(hadron_flag==1){
			T->Add(Form("../sidis_model/sidis_3he_pip_%d_0_1_0.root",energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_3he_pip_%d_0_2_0.root",energy));   // save Q2<10, pt<1
		}
		else if(hadron_flag==2){
			T->Add(Form("../sidis_model/sidis_3he_pim_%d_0_1_0.root",energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_3he_pim_%d_0_2_0.root",energy));   // save Q2<10, pt<1
		}
	}else if(particle=="kaon"){
		HADRON_CUT = 2.5;
		if(hadron_flag==1){
			T->Add(Form("../sidis_model/sidis_3he_kp_%d_0_1_0.root",energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_3he_kp_%d_0_2_0.root",energy));   // save Q2<10, pt<1
		}
		if(hadron_flag==2){
			T->Add(Form("../sidis_model/sidis_3he_km_%d_0_1_0.root",energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_3he_km_%d_0_2_0.root",energy));   // save Q2<10, pt<1
		}
	}
...
//	T->SetBranchAddress("weight_hm",&weight_hm);
	...
	//_____p vs theta
	TH2F *hf_p_theta=new TH2F("hf_p_theta","P_{ele} VS #theta_{ele}  (forward angle)",Nbin_theta,7.0,30.0,Nbin_p,0,11);
	//forward angle
	hf_p_theta->GetYaxis()->SetTitle("P_{ele}");
	hf_p_theta->GetXaxis()->SetTitle("#theta_{ele}");
	//large angle
	TH2F *hl_p_theta=new TH2F("hl_p_theta","P_{ele} VS #theta_{ele} (large angle)",Nbin_theta,7.0,30.0,Nbin_p,0,11);
	hl_p_theta->GetYaxis()->SetTitle("P_{ele}");
	hl_p_theta->GetXaxis()->SetTitle("#theta_{ele}");
	//total
	TH2F *h_p_theta=new TH2F("h_p_theta","P_{ele} VS #theta_{ele} (large+forward angle)",Nbin_theta,7.0,30.0,Nbin_p,0,11);
	h_p_theta->GetYaxis()->SetTitle("P_{ele}");
	h_p_theta->GetXaxis()->SetTitle("#theta_{ele}");

	//_____TH1F of theta
	//forward angle
	TH1F *hf_theta=new TH1F("hf_theta","rate as function of #theta_{ele}  (forward angle)",Nbin_theta,7.0,30.0);
	hf_theta->GetYaxis()->SetTitle("rate");
	hf_theta->GetXaxis()->SetTitle("#theta_{ele}");
	//large angle
	TH1F *hl_theta=new TH1F("hl_theta","rate as function of #theta_{ele} (large angle)",Nbin_theta,7.0,30.0);
	hl_theta->GetYaxis()->SetTitle("rate");
	hl_theta->GetXaxis()->SetTitle("#theta_{ele}");
	//total
	TH1F *h_theta=new TH1F("h_theta","rate as function of #theta_{ele} (large+forward angle)",Nbin_theta,7.0,30.0);
	h_theta->GetYaxis()->SetTitle("rate");
	h_theta->GetXaxis()->SetTitle("#theta_{ele}");

	//_____p vs theta
	TH2F *nf_p_theta=new TH2F("nf_p_theta","P_{had} VS #theta_{had}  (forward angle)",Nbin_theta,7.0,30.0,Nbin_p,0,11);
	//forward angle
	nf_p_theta->GetYaxis()->SetTitle("P_{had}");
	nf_p_theta->GetXaxis()->SetTitle("#theta_{had}");
	//large angle
	TH2F *nl_p_theta=new TH2F("nl_p_theta","P_{had} VS #theta_{had} (large angle)",Nbin_theta,7.0,30.0,Nbin_p,0,11);
	nl_p_theta->GetYaxis()->SetTitle("P_{had}");
	nl_p_theta->GetXaxis()->SetTitle("#theta_{had}");
	//total
	TH2F *n_p_theta=new TH2F("n_p_theta","P_{had} VS #theta_{had} (large+forward angle)",Nbin_theta,7.0,30.0,Nbin_p,0,11);
	n_p_theta->GetYaxis()->SetTitle("P_{had}");
	n_p_theta->GetXaxis()->SetTitle("#theta_{had}");

	//_____TH1F of theta
	//forward angle
	TH1F *nf_theta=new TH1F("nf_theta","rate as function of #theta_{had}  (forward angle)",Nbin_theta,7.0,30.0);
	nf_theta->GetYaxis()->SetTitle("rate");
	nf_theta->GetXaxis()->SetTitle("#theta_{had}");
	//large angle
	TH1F *nl_theta=new TH1F("nl_theta","rate as function of #theta_{had} (large angle)",Nbin_theta,7.0,30.0);
	nl_theta->GetYaxis()->SetTitle("rate");
	nl_theta->GetXaxis()->SetTitle("#theta_{had}");
	//total
	TH1F *n_theta=new TH1F("n_theta","rate as function of #theta_{had} (large+forward angle)",Nbin_theta,7.0,30.0);
	n_theta->GetYaxis()->SetTitle("rate");
	n_theta->GetXaxis()->SetTitle("#theta_{had}");
	/*}}}*/
	...
	for(Long64_t i=0;i<N_entries;i++){
		...
		if(mom_ele>=1.0&&mom_ele<=10.0&&W>=2.3&&Wp>=1.6&&z>0.3&&z<0.7&&mom_had<=HADRON_CUT){//any additional cuts should be added in here
			...
			//p vs theta
			nf_p_theta->Fill(theta_had*DEG,mom_had,event_weight*forward_acceptance);
			nl_p_theta->Fill(theta_had*DEG,mom_had,event_weight*large_acceptance);
			n_p_theta->Fill(theta_had*DEG,mom_had, event_weight*(forward_acceptance+large_acceptance));

			//TH1F theta
			nf_theta->Fill(theta_had*DEG,event_weight*forward_acceptance);
			nl_theta->Fill(theta_had*DEG,event_weight*large_acceptance);
			n_theta->Fill(theta_had*DEG,event_weight*(forward_acceptance+large_acceptance));
	/*}}}*/
		}
	...
	outf<<"ele rate_forward: "<<ele_rate_forward*Lumi*KHz*nBcm2<<endl;
	outf<<"ele rate_large: "<<ele_rate_large*Lumi*KHz*nBcm2<<endl;
	...
	gPad->SetLogz(1);
	hf_z_pt->Draw("COLZ");
	can_forward->Print(Form("CLEO_E%d_%s_%s_Forward.png",energy, particle.Data(), charge.Data()));
	...
	gPad->SetLogz(1);
	hl_z_pt->Draw("COLZ");
	can_large->Print(Form("CLEO_E%d_%s_%s_Large.png",energy, particle.Data(), charge.Data()));
	...
	can_total->Print(Form("CLEO_E%d_%s_%s_Total.png",energy, particle.Data(), charge.Data()));

	TCanvas *c1=new TCanvas("c1","theta and P_{ele} plots");
	c1->Divide(2,2);
	c1->cd(1);
	gPad->SetLogz(1);
	...
	c1->cd(4);
	hl_theta->Draw();
	c1->Print(Form("CLEO_E%d_%s_%s_eThetaP.png",energy, particle.Data(), charge.Data()));

	TCanvas *c2=new TCanvas("c1","theta and P_{had} plots");
	c2->Divide(2,2);
	c2->cd(1);
	gPad->SetLogz(1);
	nf_p_theta->Draw("COLZ");
	c2->cd(2);
	gPad->SetLogz(1);
	nl_p_theta->Draw("COLZ");
	c2->cd(3);
	nf_theta->Draw();
	c2->cd(4);
	nl_theta->Draw();
	c2->Print(Form("CLEO_E%d_%s_%s_nThetaP.png",energy, particle.Data(), charge.Data()));
	/*}}}*/
	...
	if(particle=="pion"){
		if(hadron_flag==1){
			outputfilename = Form("pip_%d_histo.root",energy);
		}
		if(hadron_flag==2){
			outputfilename = Form("pim_%d_histo.root",energy);
		}
	}

	if(particle=="kaon"){
		if(hadron_flag==1){
			outputfilename = Form("kp_%d_histo.root",energy);
		}
		if(hadron_flag==2){
			outputfilename = "km_histo.root";
			outputfilename = Form("km_%d_histo.root",energy);
		}
	}
