...
acceptance_solid_CLEO_SIDIS_nh3_negative_output.root
*/
...
int main(){
	...
	if(particle=="pion"){
		if(hadron_flag==1){
			T->Add(Form("../sidis_data/sidis_nh3_pip_%d_0_1_11.root",ele_energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_data/sidis_nh3_pip_%d_0_2_11.root",ele_energy));   // save Q2<10, pt<1
		}
		else if(hadron_flag==2){
			T->Add(Form("../sidis_data/sidis_nh3_pim_%d_0_1_11.root",ele_energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_data/sidis_nh3_pim_%d_0_2_11.root",ele_energy));   // save Q2<10, pt<1
		}
	}else if(particle=="kaon"){
		if(hadron_flag==1){
//			T->Add(Form("./sidis_nh3_kp_%d_0_1_0.root",ele_energy));   // save Q2<10, pt<1
//			T->Add(Form("./sidis_nh3_kp_%d_0_2_0.root",ele_energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_nh3_kp_%d_0_1_0.root",ele_energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_nh3_kp_%d_0_2_0.root",ele_energy));   // save Q2<10, pt<1
		}
		if(hadron_flag==2){
//			T->Add(Form("./sidis_nh3_km_%d_0_1_0.root",ele_energy));   // save Q2<10, pt<1
//			T->Add(Form("./sidis_nh3_km_%d_0_2_0.root",ele_energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_nh3_km_%d_0_1_0.root",ele_energy));   // save Q2<10, pt<1
			T->Add(Form("../sidis_model/sidis_nh3_km_%d_0_2_0.root",ele_energy));   // save Q2<10, pt<1
			}
	}
	...
	for(Long64_t i=0;i<N_entries;i++){
		...
		if(mom_ele>=1.0&&mom_ele<=10.0&&W>=2.3&&Wp>=1.6&&z>0.3&&z<0.7&&mom_had<=2.5){//any additional cuts should be added in here
			...
			int ele_phi_bin=int(phi_ele/3.14159265*180.0/2.0)+1;      //2.0 degree per bin for phi
			int ele_p_bin=int(mom_ele/0.05)+1;      //0.05 GeV per bin for mom
			double ele_forward_acceptance=accep_ele_forward->GetBinContent(ele_theta_bin,ele_phi_bin,ele_p_bin);
			double ele_large_acceptance=accep_ele_large->GetBinContent(ele_theta_bin,ele_phi_bin,ele_p_bin);

			int hadron_theta_bin=int(theta_had/3.14159265*180.0/0.2)+1;  //0.2 degree per bin
			int hadron_phi_bin=int(phi_had/3.14159265*180.0/2.0)+1;  //2.0 degree per bin
			int hadron_p_bin=int(mom_had/0.05)+1;     //0.05 GeV per bin
			double hadron_acceptance=0;
			//In SIDIS, hadron are detected only in the Forward-Angle
			hadron_acceptance=accep_had->GetBinContent(hadron_theta_bin,hadron_phi_bin,hadron_p_bin);
