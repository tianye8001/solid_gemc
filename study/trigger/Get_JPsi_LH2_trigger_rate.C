...
#include "../../solid_gemc2/analysis/analysis_tree_solid_lgc.C"
#include "../../solid_gemc2/analysis/analysis_tree_solid_spd.C"
#include "../../solid_gemc2/analysis/analysis_tree_solid_mrpc.C"
...
int Get_JPsi_LH2_trigger_rate(string inputfile_name,bool Is_tellorig=false,string filetype=""){
...
// 	    	cout << " hit_phi " << hit_phi << " sec_ec " << sec_ec << endl;	
		    ...
		    //using SIDIS_He3 trigger for now
		    double hit_r_tmp;
		    if (105<=hit_r && hit_r<115) hit_r_tmp=110;     //trigger at 4GeV
		    else if(115<=hit_r && hit_r<145) hit_r_tmp=120; //trigger at 3GeV
		    else if(145<=hit_r && hit_r<235) hit_r_tmp=140; //trigger at 2GeV
	    ...
	    // 	cout << " hit_phi " << hit_phi << " sec_ec " << sec_ec << endl;	
		    ...
		    //using SIDIS_He3 trigger for now
		    double hit_r_tmp;
		    if (80<=hit_r && hit_r<100) hit_r_tmp=120;     //trigger at 3GeV
		    else if(100<=hit_r && hit_r<140) hit_r_tmp=140; //trigger at 2GeV
		    else {hit_r_tmp=0; cout << "hit_r " << hit_r << " out of range"  << endl;}
...
// 		int trigger_mrpc_FA[50][3]={0};		
		...
		int i_e_FA_EC_good[100]={0},i_e_LA_EC_good[100]={0};
...
// 			    cout << "event " << i<< " counter_e_LA " <<  counter_e_LA_EC_spd << "\t" << trigger_e_LA_EC_pid[i_e_LA_EC] << " " << trigger_e_LA_EC_p[i_e_LA_EC] << " " << trigger_e_LA_EC_sec[i_e_LA_EC] << " " << 	trigger_e_LA_EC_r[i_e_LA_EC] << endl;			   
		...
		//coin trigger
		int counter_e_e=(counter_e_FA_EC_lgc_spd_mrpc+counter_e_LA_EC_spd)*(counter_e_FA_EC_lgc_spd_mrpc+counter_e_LA_EC_spd-1);
		if (counter_e_e != 0) coin_rate += rate/counter_e_e;	
...
cout<<"only h_trigger_e_FA_EC: "<<h_trigger_e_FA_EC->Integral(1,60)/1e3<<endl;
cout<<"only h_trigger_e_LA_EC: "<<h_trigger_e_LA_EC->Integral(1,60)/1e3<<endl;
cout<<"only h_trigger_h_FA_EC: "<<h_trigger_h_FA_EC->Integral(1,60)/1e3<<endl;
cout<<"only lgc: "<<h_n_trigger_sectors_LGC->Integral(1,30)/1e3<<endl;
cout<<"only spd FA: "<<h_n_trigger_sectors_spd_FA->Integral(1,240)/1e3<<endl;
cout<<"only spd LA: "<<h_n_trigger_sectors_spd_LA->Integral(1,240)/1e3<<endl;
cout<<"only mrpc: "<<h_n_trigger_sectors_mrpc->Integral(1,240)/1e3<<endl;
// cout<<"EC lgc spd mrpc be fired: "<<h_flux_EC_lgc_spd_mrpc->Integral(1,60)<<endl;	
...
cout<<"only trigger_e_e: "<<coin_rate/1e3<< endl; 
