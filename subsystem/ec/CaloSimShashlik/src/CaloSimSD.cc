	...
	G4VSensitiveDetector(name)
{
	...
	T->Branch("blockfiberEdep_PS",blockfiberEdep_PS,"blockfiberEdep_PS[NBlockHit]/D");
...
//	T->Branch("blockfiberEdepLayer",blockfiberEdepLayer,"blockfiberEdepLayer[NBlockHitLayer]/D");
	...
	T->Branch("fiberEdepIonizingLayer",fiberEdepIonizingLayer,"fiberEdepIonizingLayer[NHitLayer]/D");
	T->Branch("WEdepLayer",WEdepLayer,"WEdepLayer[NHitLayer]/D");


	T->Branch("blockfiberPhotons",blockfiberPhotons,"blockfiberPhotons[NBlockHit]/D");
	T->Branch("fiberPhotonsLayer",fiberPhotonsLayer,"fiberPhotonsLayer[NHitLayer]/D");
...
G4bool CaloSimSD::ProcessHits(G4Step* aStep, G4TouchableHistory* /*ROhist*/)
{
	...
	newHit->SetEdepIonizing(aStep->GetTotalEnergyDeposit() - aStep->GetNonIonizingEnergyDeposit());
...
void CaloSimSD::ProcessEventHC(G4HCofThisEvent* hitCollections)
{
	...
	for(int i=0; i<nblock; i++)
	{
		...
		blockWEdep[i] = blockglueEdep[i] = blockfiberEdep[i] =blockfiberEdep_PS[i]= blockfiberPhotons[i] =0;
...
//			G4int thisType = (*aHitCollWithinThisEvent)[ent]->GetParticleType();
			...
			G4double edep_ionizing = (*aHitCollWithinThisEvent)[ent]->GetEdepIonizing();
			...
			G4ThreeVector hitpos =  (*aHitCollWithinThisEvent)[ent]->GetHitPosition();
...
//			// std::cerr<<"Test Process ..."<<G4endl;
			...
			if (!strcmp(thisVolume, "CaloBlock"))
			{
				...
				G4int Wlayer = floor((hitpos.z() -
						leading_Pb - leading_Scint)/layerSep)+1;
				if (Wlayer<0) Wlayer = 0;
...
//	G4cout<<"caloX= "<<caloX/cm+7.5<<"   caloY= "<<caloY/cm+7.5<<"   caloZ= "<<caloZ/cm<<G4endl;
	...
	for(int i=0; i<nblock; i++)
	{
		...
		for (int j=0;j<nScintlayer;j++)
		{
			blockfiberPhotonsLayer[i*nScintlayer + j] =
					RndGen.Poisson(blockfiberPhotonsLayer[i*nScintlayer + j]);
		}
	}
	for (int j=0;j<nScintlayer;j++)
	{
	...
	G4cout << "Sampling ratio = "
			<<fiberEdep / (fiberEdep + glueEdep + WEdep) << ", "
			<<fiberEdepIonizing / (fiberEdep + glueEdep + WEdep) << " (Ionizing)"
			<<G4endl;
}
