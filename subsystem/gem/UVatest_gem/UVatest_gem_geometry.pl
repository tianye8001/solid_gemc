...
my $DetectorName = 'UVatest_gem';

my $DetectorMother="root";

sub UVatest_gem_geometry
{
make_gem();
}

my $Nplate	= 1;
my @PlateX = (0);  #position in X, unit cm
my @PlateY = (0);  #position in Y, unit cm
my @PlateZ = (100);  #position in Z, unit cm
my @Dx = (100/2);    #half length in X, unit cm
my @Dy = (100/2);     #half length in Y, unit cm
...
# =====SoLID GEM =================================
# refer  to https://hallaweb.jlab.org/wiki/index.php/Solid_GEM_Digitization#GEM_module.27s_geometry_and_material
...
# my @hittype = ("no","no","no","no","flux","flux","flux","no","flux","flux","flux","no","no","no","no","no","no","no","flux","no","no","no","no","no");
 ...
 for(my $n=1; $n<=$Nplate; $n++)
 {
    ...
    $detector{"pos"}        = "$PlateX[$n-1]*cm $PlateY[$n-1]*cm $PlateZ[$n-1]*cm";
...
# 	    $detector{"dimensions"} = "$Rin[$n-1]*cm $Rout[$n-1]*cm $DlayerZ*mm $angle_start*deg $angle_width*deg";
	    $detector{"type"}       = "Box";
	    $detector{"dimensions"} = "$Dx[$n-1]*cm $Dy[$n-1]*cm $DlayerZ*mm";
