...
//	const Int_t GEM_VP_ID[GEM_Plane] = {1110000,1210000,1310000,1410000,1510000,1610000,}; //GEM virtual plane
	...
	TString output_filename = Form("GEM_%s.root",pid_temp.Data() );
	...
	/*Read in each event{{{*/
	ofstream laoutfile(Form("SoLID_GEM_LA_%s.dat",pid_temp.Data()));
	...
	laoutfile <<Form("  GEM#1: Plane, PID, Px, Py, Pz, X, Y, Z, Mom, Theta, Phi, E, Edep")<<endl;	
	laoutfile <<"  ..."<<endl;
	laoutfile <<Form("  GEM#4: Plane, PID, Px, Py, Pz, X, Y, Z, Mom, Theta, Phi, E, Edep")<<endl;	
	laoutfile <<Form("  LAEC:  EC-ID, PID, Px, Py, Pz, X, Y, Z, Mom, Theta, Phi, E")<<endl;	
	laoutfile <<Form("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")<<endl;	

	ofstream faoutfile(Form("SoLID_GEM_FA_%s.dat",pid_temp.Data()));
	...
	faoutfile <<Form("  GEM#1: Plane, PID, Px, Py, Pz, X, Y, Z, Mom, Theta, Phi, E, Edep")<<endl;	
	faoutfile <<"  ..."<<endl;
	faoutfile <<Form("  GEM#6: Plane, PID, Px, Py, Pz, X, Y, Z, Mom, Theta, Phi, E, Edep")<<endl;	
	faoutfile <<Form("  FAEC:  EC-ID, PID, Px, Py, Pz, X, Y, Z, Mom, Theta, Phi, E")<<endl;	
	faoutfile <<Form("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")<<endl;	
...
//					&&PID_flux!=Kaon&&PID_flux!=-Kaon) continue;
		...
		if(Is_LAEC_Hit && Count_GEM_Hit >=3 && !(Is_GEM_Hit[4]) && !(Is_GEM_Hit[5]) ){
			...
			laoutfile <<Form("%2d %5d %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e", 
			        310, LAEC_PID,
		...
		if(Is_FAEC_Hit&&Count_GEM_Hit>=5){
			...
			faoutfile <<Form("%2d %5d %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e %10.2e", 
			        320, FAEC_PID,
