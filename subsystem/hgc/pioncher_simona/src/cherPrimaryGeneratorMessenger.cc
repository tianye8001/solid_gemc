...
//
//
// $Id: cherPrimaryGeneratorMessenger.cc,v 1.3 2006/06/29 17:54:29 gunter Exp $
...
cherPrimaryGeneratorMessenger::cherPrimaryGeneratorMessenger(
                                          cherPrimaryGeneratorAction* cherGun)
:cherAction(cherGun)
{
...
void cherPrimaryGeneratorMessenger::SetNewValue(
                                        G4UIcommand* command, G4String newValue)
{ 
  if( command == polarCmd ) {
      G4double angle = polarCmd->GetNewDoubleValue(newValue);
      if ( angle == -360.0*deg ) {
         cherAction->SetOptPhotonPolar();
      } else {
         cherAction->SetOptPhotonPolar(angle);
      }
  }
}
